...
TBD: Describe arithmetic operators.
...
constructing a record. For example:
  ...
  item = (drinkname = "latte", price = 2.0 +. 0.5)          # OK
  item = ("drink" + "name" = "latte", price = 2.0 +. 0.5)   # NOT OK
...
You can also add a field to an arbitrary record using the I<record
extension> operation.

  (caffeineContent = 60 | item)

With the previous definition, this yields a value
...
by wrapping a comma-delimited series of expressions in brackets:
...
TBD: where and orderby

=head2 Tables

In order to access a database table it must be declared at the
...
declares a table wordlist with fields: word, type and meaning all of
type String. The database descriptor contains a number of fields
separated by colons:

 driver:name:host:port:user:password
...
many threads by executing the C<spawn> primitive:
  ...
  var newThreadID = spawn(func, arg)
...
Currently, the behavior is quite different between programs that have
at least one client-annotated function, and those that have none at
all. In the former case, the program is downloaded to the client as a
stub that initiates the computation. The initialization part of the
program may substantially take place on the server, but it will be
initiated by the client as a client-to-server call. (This is a
by-product of the current implementation, which only allows
server-to-client calls in the context of an existing client-to-server
call.)

If you have no "client" annotations at all, however, the page is
completely computed at the server and downladed as HTML per se. This
does not preclude having code that runs on the client, for example
user-event handlers embedded in HTML elements; this only prevents you
from having named functions with the explicit "client" keyword.
...
=item C<eventGetTarget>

An event generally has a I<target>, a DomRef at which the event has 
...
=item C<eventGetTargetResolveTextNode>

This method behaves like C<eventGetTarget>, except that if the node is a text 
node (rather than an element node), it returns the node's parent, which must 
be an element node.

(TBD: rename this to C<eventGetTargetElement>)

=item C<eventGetRelatedTarget>
...
=item C<eventGetPageX>

=item C<eventGetPageY>
...
screen. Also: how about C<eventGetPageXY> returning an (x, y) pair?)
...
that module, using objects call C<DomRef>s. You can create DOM
nodes by passing an XML tree: the XML constitutes a model for the DOM
node, which is a rough copy of the XML. Likewise, from a DomRef,
...
Replaces the children of C<ref> with xmlVal.

=item C<domSwapNodeRefs(ref1, ref2)>

Swap ref1 with ref2.


=item C<domGetDocumentRef()>
...
=item C<domGetXml(ref)>

Translates the given DOM node into an XML value and returns that.

=item C<domIsNullRef(ref)>

Returns true if C<ref> is null.
...
=item domGetAttributeFromRef(ref, attrName)
...
(TBD: Document XML accessors getTagName, getAttribute, getTextContent, 
getAttributes)

(Note: The following are unimplemented as yet: domGetChildrenRef,
getChildren, domGetFirstChildRef, domGetNextSiblingRef,
domGetPrevSiblingRef)
...
From the command line Links can be invoked in a number of different ways:
  ...
  links            load the interactive environment
...
=head2 The interactive interpreter

The interactive interpreter allows server-side code to be entered
...
The interpreter supports a number of directives.

 @directives           : list available directives
 @settings             : print available settings
 @set                  : change the value of a setting
 @builtins             : list builtin functions and values
