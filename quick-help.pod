...
write:
  ...
  var db = database (name="factorials", driver="mysql", args=":3306:web:");
...
Inside the generator of a C<for> comprehension C<r <-- t> is syntactic
sugar for C<r <- asList(t)>. So you can write:

  for (r <-- factorials)
   where (i <= 20)
   [r]

=head2 Databases modification
