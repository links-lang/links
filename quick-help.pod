...
{ } will be evaluated to an XML forest which is included when the quasi
is evaluated.
...
when the form is submitted.
   ...
   <form l:onsubmit="{say_hello(name)}>
      What is your name? 
      <input l:name="{name}"/>
   </form>

The "name" variable bound by the C<l:name> attribute is in scope in
...
each. There are a variety of tricks for getting what you want out of a
...
=item C<eventGetTargetElement>

This method behaves like C<eventGetTarget>, except that if the node is
...
Besides their "target," some events have a I<related target>. For
example, on a mouse-out event, there is an element which the mouse
moved I<out of> and one which it moved I<into>. The related target in
this case is the one which the mouse moved into.
...
Creates a node using C<xmlVal> as the image, and adds it as the
previous sibling of the node C<beforeRef>.

=item C<domAppendChild(xmlVal, parentRef)>

Creates a node using xmlVal as the image, and adds it as the last
child of the node C<parentRef>.
...
Removes C<ref> from its current position, leaving it with no
...
=item getChildren(xml)

Returns a list of the child nodes of this xml node (NOT YET IMPLEMENTED).
...
along with their types, can be obtained by typing "@builtins;" in the
interactive shell.
