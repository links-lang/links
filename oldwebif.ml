...
let get_remote_call_args lookup cgi_args = 
  ...
  let args = untuple (Oldjson.parse_json args) in
...
let expr_eval_req valenv program params =
  ...
  let mkStringPair (l, r) =
    Syntax.Record_intro (StringMap.from_alist 
                         ...
                         None, data) in
    match Result.unmarshal_result (range valenv) program  (List.assoc "_k" params) with
      | `RecFunction ([(_,f)],locals,_) ->
          let json_args = try (match Oldjson.parse_json_b64 (List.assoc "_jsonArgs" params) with
...
(*                                          env, data)) *)
(*                        env (Syntax.Nil data)) in *)
            ExprEval (Syntax.Apply (f, [], data), locals @ json_args)
      | _ -> assert false
...
let client_return_req cgi_args = 
  ...
  let arg = Oldjson.parse_json_b64 (List.assoc "__result" cgi_args) in
...
let perform_request 
  ...
  match req with
    ...
    | ClientReturn(cont, value) ->
        ...
        let result_json = (Oldjson.jsonize_result 
    ...
    | RemoteCall(func, args) ->
	  ...
	  Library.print_http_response [("Content-type", "text/plain")]
            (Utility.base64encode (Oldjson.jsonize_result result))
    | CallMain -> 
        Library.print_http_response [("Content-type", "text/html")] 
  ...
  try 
    ...
    let request = 
      if is_remote_call cgi_args then
        ...
        contin_invoke_req (range valenv) program cgi_args
