...
type 'data expression' =
  ...
  | Condition of ('data expression' * 'data expression' * 'data expression' * 'data)
  ...
  | Xml_text of string * 'data
  | Xml_element of (string * (string * 'data expression') list * 'data expression' list * 'data)
  | Xml_identity of ('data expression' * 'data) (* Should appear only temporary during type-inference... *)
  | Record_empty of 'data
  | Record_extension of (string * 'data expression' * 'data expression' * 'data)
  | Record_selection of (string * string * string * 'data expression' * 'data expression' * 'data)
  | Record_selection_empty of ('data expression' * 'data expression' * 'data)
  | Variant_injection of (string * 'data expression' * 'data)
  | Variant_selection of ('data expression' * string * string * 'data expression' * string * 'data expression' * 'data)
  ...
  | TableQuery of ((string * 'data expression') list * Query.query * 'data)
  | TableHandle of ('data expression' * 'data expression' * Types.row * 'data)
  ...
  | PatternHasType of ('data expression' * Types.datatype * 'data) (* Should disappear after type inference *)
...
let rec show t : 'a expression' -> string = function 
  ...
  | PatternHasType(expr, datatype, data) -> show t expr ^ " : " ^ Types.string_of_datatype datatype ^ t data
  ...
  | Xml_identity (expr, _) -> show t expr
  | Xml_text (s, data) -> "\"" ^ s ^ "\"" ^ t data
...
let reduce_expression (visitor : ('a expression' -> 'b) -> 'a expression' -> 'b)
  ...
  let rec visit_children expr = 
    combine (expr, match expr with
               ...
               | PatternHasType (e, _, _) -> [visitor visit_children e]
...
let expression_data : ('a expression' -> 'a) = function 
        ...
        | Let (_, _, _, data) -> data
        | Rec (_, _, data) -> data
        | Xml_identity (_, data) -> data
        | Xml_text (_, data) -> data
