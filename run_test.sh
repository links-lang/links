...
echo "Test 7"
_build/default/bin/links.exe --mode wasm <(echo 'fun f(x : Int) { fun g() { x } g } f(1)()') -o output_test.wast || exit $?
echo '(assert_return (get "_init_result") (i64.const 1))' >> output_test.wast
../stack-switching/interpreter/wasm output_test.wast || exit $?

echo "Test 8"
_build/default/bin/links.exe --enable-handlers --mode wasm <(echo 'handle (5) { case <Foo => k> -> 2 }') -o output_test.wast || exit $?
echo '(assert_return (get "_init_result") (i64.const 5))' >> output_test.wast
../stack-switching/interpreter/wasm output_test.wast || exit $?
_build/default/bin/links.exe --enable-handlers --mode wasm <(echo 'handle (do Foo) { case <Foo => k> -> 2 }') -o output_test.wast || exit $?
echo '(assert_return (get "_init_result") (i64.const 2))' >> output_test.wast
../stack-switching/interpreter/wasm output_test.wast || exit $?

echo "Test 9"
_build/default/bin/links.exe --enable-handlers --mode wasm <(echo 'handle (()) { case <Foo => k> -> k(()) }') -o output_test.wast || exit $?
echo '(assert_return (get "_init_result") (i32.const 0))' >> output_test.wast
../stack-switching/interpreter/wasm output_test.wast || exit $?
_build/default/bin/links.exe --enable-handlers --mode wasm <(echo 'handle (do Foo) { case <Foo => k> -> k(()) }') -o output_test.wast || exit $?
echo '(assert_return (get "_init_result") (i32.const 0))' >> output_test.wast
../stack-switching/interpreter/wasm output_test.wast || exit $?
