...
let xml_type xml_type = `Primitive (`XML xml_type)
...
let rec type_of_inference_type : type_var_set -> datatype -> Types.datatype = fun rec_vars ->
  function
    ...
    | `Primitive `Float -> `Primitive `Float
    | `Primitive `Int -> `Primitive `Int
    | `Primitive (`XML xml_type) -> `Primitive (`XML (Xml.Inference.to_type xml_type))
