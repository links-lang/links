...
/* genEventHandler: Takes a target, a Links variant type of `MvuEvents.EventHandler`,
 ...
 *         handler: Links MvuEvents.EventHandler
 * Output: { evtName |-> [fn] }
 */
function genEventHandler(target, evtHandler) {
  ...
  if (evtTag == "MvuEvents.UnitHandler") {
    ...
    return retEvents;
  } else if (evtTag == "MvuEvents.PropertyHandler") {
    const evtName =  handler["1"];
    ...
    ret[evtName] = [fn];
    return ret;
  } else if (evtTag == "MvuEvents.MouseEventHandler") {
    ...
    return retEvents;
  } else if (evtTag == "MvuEvents.KeyboardEventHandler") {
    const evtName = handler["1"];
    const genMsgFn = handler["2"]
    const retEvents = {};
    const fn =
