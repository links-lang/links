...
function _makeParamFun(penv, h, s) {
    return function() {
        const op_arg = arguments[0];
        const ks = arguments[arguments.length - 1];
        const param_args = Array.prototype.slice.call(arguments, 1, -1);
        for (let i = 0; i < param_args.length; i++) {
            penv[i] = param_args[i];
        }

        return _yieldCont(LINKEDLIST.revAppend(_Cons(h, s), ks), op_arg);
    }
}

function _makeFun (h, s) {
  return function (x, ks) {
    return _yieldCont(LINKEDLIST.revAppend(_Cons(h, s), ks), x);
  }
}
