...
var LINKS = function() {
  ...
  var _remoteCallHandler = function (kappa, request) {
    return function() {
      if (request.readyState == AJAX.isComplete) {
       // FIXME: this case apparently triggers if we leave the page.
       // AHA:: Here's the bug; we have extra escapes in this string.
  ...
  var LINKS = {
    ...
    XmlToDomNodes : function (xmlForest) {
      DEBUG.assert(DEBUG.is_array(xmlForest),
                   'LINKS.XmlToDomNodes expected an array, but got ' + xmlForest);
    ...
    project : function(name, object) { return object[name]; },
    ...
    remoteCall : function(kappa) {
     return function(name, arguments) {
       ...
       // make sure the correct pid is set
       // for kappa!
