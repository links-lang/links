...
function _charlistToString(chlist) {
  DEBUG.assert(DEBUG.is_array(chlist), "Expected an array, got: " + chlist);
  var str = "";
  for (var i in chlist) {
    str += chlist[i]
  }
...
var _plus = _continuationizeBinaryCurried(__plus);
var _times = _continuationizeBinaryCurried(__times);
var _divide = _continuationizeBinaryCurried(__divide);
var _minus = _continuationizeBinaryCurried(__minus);
var _hyphen = _minus;
var _star = _times;
var _slash = _divide;
...
function _remove(object, name) {
  // Surely this function is unnecessary?
  // Links is a functional language after all!
...
//  alert("invokeClientCall: _f is " + _callPackage.__name);
...
//  alert("invokeClientCall: its value is " + _f);
  ...
  _f.apply(_f, [_callPackage.__arg, _cont]);
