...
var WEBSOCKET = function() {
  return {
    ...
    sendRemoteAPRequest : function(current_pid, remote_apid) {
      _debug("Requesting a connection from remote AP " + remote_apid);
      var to_send_json =
    ...
    sendRemoteAPAccept : function(current_pid, remote_apid) {
      _debug("Accepting a connection from remote AP " + remote_apid);
      var to_send_json =
...
function blockUntilAPResponse(remote_apid, kappa) {
  // Block the process until the response has been received
  _block_proc(
    current_pid,
    function () {
      // Grab the returned channel EP out of the _returned_channels table,
      // and continue
      DEBUG.assert(_returned_channels[remote_apid] != undefined,
        "resuming process after remote accept, but no channel available!");
      var chan = _returned_channels[remote_apid];
      delete _returned_channels[remote_apid];
      kappa(chan);
    }
  );
}
...
function localAccept(localAPID, kappa) {
  ...
  if (_aps[localAPID].requesters.length > 0) {
    r = _aps[localAPID].requesters.pop();
    _channels[r.channel][0] = {proc: _current_pid, messages: []}
    _wakeup(r.proc);
    kappa({channel: r.channel, direction: 0});
  } else {
    // If not, then create a new channel, push the request onto the accepters stack and block
    // the current process until a requester becomes available.
    var current_pid = _current_pid;
    var channel = _nextChannel;
    _nextChannel = _nextChannel + 1;
    _channels[channel] = [{proc: _current_pid, messages: []}, {proc: 0, messages: []}]
    _aps[localAPID].accepters.unshift({proc: current_pid, channel: channel});
...
function localRequest(ap, kappa) {
  _debug("Request called on AP " + ap);
...
function remoteRequest(remote_apid, kappa) {
  // Request remotely
  WEBSOCKET.sendRemoteAPRequest(current_pid, remote_apid);
  blockUntilAPResponse(remote_apid, kappa);
}

function request(ap, kappa) {
  if (_is_valid_server_ap(ap)) {
    remoteRequest(ap, kappa);
  } else if (_is_valid_client_ap(ap)) {
    if (get_client_id_from_ap(ap) != _client_id) {
