...
const DEBUG = Object.freeze({
  debug: function (...msg) { DEBUGGING && console.debug(...msg) },
...
const WEBSOCKET = {
      ...
      if ("opcode" in js_parsed) {
        switch (js_parsed.opcode) {
          case "MESSAGE_DELIVERY":
            ...
            const local_pid = js_parsed.dest_pid;
            const msg = js_parsed.val;
            LINKS.deliverMessage(local_pid, msg);
            break;
          case "AP_RESPONSE":
            const blocked_pid = js_parsed.blocked_pid;
            const chan = js_parsed.chan;
            _returned_channels[blocked_pid] = chan;
            _buffers[chan._sessEP2] = [];
            _wakeup(blocked_pid);
            break;
          case "SESSION_MESSAGE_DELIVERY":
            const ep_id = js_parsed.ep_id;
            const msg = js_parsed.msg;
            const delegated_chans = js_parsed.deleg_chans;
            const requires_lost_messages = js_parsed.requires_lost_messages;
            migrateDelegatedSessions(delegated_chans, requires_lost_messages);
            deliverSessionMessage(ep_id, msg);
            break;
          case "GET_LOST_MESSAGES":
            const remote_ep = js_parsed.carrier_ep;
            const ep_ids = js_parsed.ep_ids;
            handleGetLostMessages(remote_ep, ep_ids);
            break;
          case "DELIVER_LOST_MESSAGES":
            const ep_id = js_parsed.ep_id;
            const lost_message_table = js_parsed.lost_messages;
            handleLostMessages(ep_id, lost_message_table);
            break;
          case "CHANNEL_CANCELLATION":
            const notify_ep = js_parsed.notify_ep;
            const cancelled_ep = js_parsed.cancelled_ep;
  ...
  sendLostMessageResponse : function (ep_id, lost_message_table) {
    DEBUG.debug("Delivering lost messages for remote EP ", ep_id, ", message table: "
...
const LINKS = new (function() {
 ...
 /**
  ...
  function _invokeClientCall (kappa, callPackage) {
    ...
    _yield(function () { return f.apply(f, append(args, k)) });
  }
  ...
  const LINKS = {
    ...
    XmlToDomNodes : function (xmlForest) {
      DEBUG.assert(TYPES.isArray(xmlForest),
    ...
    XMLk : function (tag, attrs, body, kappa) {
      _applyCont(kappa, LINKS.XML(tag, attrs, body));
    },
    ...
    deliverMessage: function(pid, msg) {
      ...
      _wakeup(pid);
      DEBUG.debug(pid, ' now has ', _mailboxes[pid].length, ' message(s)');
    },
...
function _domReplaceChildren(xml, parent) {
  ...
  newNodes.forEach(function (node) { _domAppendChildRef(node, parent) });
  return CONSTANTS.UNIT;
}
...
function _replaceDocument(tree) {
  ...
  DEBUG.assert(tree[0].type === "ELEMENT",
...
function _handleSessionException(cont_variables, return_variables) {
  const return_set = new Set(return_variables);
  const cont_set = new Set(cont_variables);

  const resolved_return_set = resolveSet(return_set);
  const resolved_cont_set = resolveSet(cont_set);

  const final_cont_set = new Set(Array.from(resolved_cont_set).filter(x => (!resolved_return_set.has(x))));
  const affected_channels = Array.from(final_cont_set);
...
function _makeFun (s) {
  return function (x, ks) {
    _yieldCont(_lsRevAppend(s, ks), x)
  }
}
...
function spawnAt(f, loc, kappa) {
    _applyCont(kappa,_spawnAt(f, loc));
}
...
function spawnWait(f, kappa) {
  ...
  _current_pid = childPid;
    ...
    f(function(v) {_current_pid = parentPid; delete _mailboxes[childPid]; _applyCont(kappa,v)});
}
...
function Send(pid, msg, kappa) {
    _applyCont(kappa,_Send(pid, msg));
}
...
function wakeupFromChan(chEP) {
  DEBUG.assert(_chan_blocked[chEP] != undefined,
...
function localRequest(localAPID, kappa) {
  ...
  switch (ap.ap_state) {
    ...
    case ACCEPTING:
      ...
      _applyCont(kappa,makeFlippedChan(top));
      break;
   default:
     DEBUG.assert(false, "Invalid access point state! " + ap.ap_state);
  }
...
function receive(c, cancellation_thunk, kappa) {
  ...
  if(isEndpointCancelled(peer_ep)) {
    ...
    cancellation_thunk();
    // Return -- we'll invoke exn handler in the thunk.
    return;
  }
...
function _applyCont_Default(k, arg) { k(arg) }
...
function childNodes(elem, kappa) {
    _applyCont(kappa,_childNodes(elem));
}
...
function textContent (node, kappa) {
    _applyCont(kappa,_textContent(node));
}
...
function sleep(duration, kappa) {
  var current_pid = _current_pid;
    setTimeout(function() { _current_pid = current_pid; _applyCont(kappa,{}); }, duration);
}
...
function _variantToXmlItem (variant) {
  ...
  if (variant['_label'] === 'Node' || variant['_label'] === 'NsNode') {
    variant['_value'][2]
      ...
      .forEach(function (a) { attrs[a['_value'][1]] = a['_value'][2]; });
    variant['_value'][2]
      .filter(function (c) { return (c['_label'] === 'NsAttr'); })
      .forEach(function (a) { attrs[a['_value'][1] + ':' + a['_value'][2]] = a['_value'][3]; });
  }
