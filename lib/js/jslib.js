  ...
  switch(expr._label){
    ...
    case "Repeat":{
      ...
      switch(expr._value[1]._label){
        ...
        default: throw("Error: Unrecognised Operation");
      }
    ...
    case "Group":{
      return "(" + _regexCompile(expr._value) + ")";
    }
    case "Quote":{
      return "\\" + _regexCompile(expr._value);
    }
    case "Replace":{
      throw("Error: \"Replace\" not implemented");
    }
    default: throw("Unrecognised Tag in Regex Expression");
  }
  return null;
}

/*const _OpCode = (function() {
  return {
      'GROUP': 0, // Emit '(' <Pop 1 operand> ')'
      'SEQ': 1,   // n = <Pop 1 instruction>; Emit <Pop n operands>
      'ALT': 2,
      'RANGE': 3
  };
})();

function _regexCompile(obj){ 
  const stack = [obj];
  const operands = [];
  const instr = [];

  // Translator to flatten the object representation
  while (stack.length > 0) {
    ...
    switch(expr._label){
      ...
      case "Seq": {
        ...
        LINKEDLIST.forEach(expr._value, function(obj){
          n++;
          stack.push(obj);
          return null;
        });
        ...
        instr.push(n);
        instr.push(_OpCode.SEQ);
        break;
      }
      case "Simply": {
        operands.push(expr._value);
        break;
      }
      case "Alternate": {
        stack.push(expr._value[1]);
        stack.push(expr._value[2]);
        instr.push(_OpCode.ALT);
        break;
      }
      case "Group": {
        stack.push(expr._value);
        instr.push(_OpCode.GROUP);
        break;
      }
    }
  }

  // Machine to assemble the regex
  while (instr.length > 0){
    switch(instr.pop()){
      case _OpCode.SEQ: {
        ...
        const ops = [];
        for(let i = 0; i < n; i++){
          ops.push(operands.pop());
        }
        operands.push(ops.join(""));
        break;
      }
      case _OpCode.ALT: {
  ...
  if(operands.length == 1){
    ...
    throw("Operands is not a singleton");
  }
