  ...
  return Object.freeze({
    ...
    equal: function(l,r) {
      ...
      while(!LINKEDLIST.isNil(curL) && !LINKEDLIST.isNil(curR)) {
        if (!LINKS.eq(curL._head, curR._head)) return false;
        curL = curL._tail;
        curR = curR._tail;
...
const WEBSOCKET = {
  ...
  serialise_and_send: function (msg) {
    _socket.send(LINKS.stringify(msg));
    return;
  },
...
const LINKS = new (function() {
  ...
  LINKS.eq = function(l, r) {
    ...
    if (TYPES.isNumber(l) && TYPES.isNumber(r)
        || TYPES.isString(l) && TYPES.isString(r)
        || TYPES.isBoolean(l) && TYPES.isBoolean(r)) return false;
    ...
    if (TYPES.isUnit(l) && TYPES.isUnit(r)) return true;

    if (TYPES.isArray(l) && TYPES.isArray(r)) {
    ...
    if (TYPES.isLinkedList(l) && TYPES.isLinkedList(r)) return LINKEDLIST.equal(l,r);
    ...
    if(typeof(l) == 'object' && l != null &&
      ...
      for (let p in l)
        if (!LINKS.eq(l[p], r[p])) return false;
...
var _$$hd = LINKEDLIST.head;
var _$$tl = LINKEDLIST.tail;
...
function _getValue(nodeRef) {
    function toDomNode(nodeRef) {
        if (nodeRef.nodeType == document.TEXT_NODE) {
            ...
            for (let i = numChildren - 1; 0 <= i; i--) {
                children = LINKEDLIST.Cons(toDomNode(nodeRef.childNodes[i]), children);
            }
            ...
            DEBUG.assert(
                !LINKEDLIST.some(function (e) { return e.type !== 'ELEMENT' && e.type !== 'TEXT'; }, children),
    ...
    return LINKEDLIST.Cons(toDomNode(nodeRef), LINKEDLIST.Nil);
}
...
function _domGetChildrenFromRef (nodeRef) {
  return LINKEDLIST.lsFromArray(nodeRef.children);
}
...
const domGetNodeValueFromRef = LINKS.kify(_domGetNodeValueFromRef)
const domGetChildrenFromRef = LINKS.kify(_domGetChildrenFromRef);
const domGetTagNameFromRef = LINKS.kify(_domGetTagNameFromRef);
const domHasAttribute = LINKS.kify(_domHasAttribute);
const domGetAttributeFromRef = LINKS.kify(_domGetAttributeFromRef);
const domSetAttributeFromRef = LINKS.kify(_domSetAttributeFromRef);
const domRemoveAttributeFromRef = LINKS.kify(_domRemoveAttributeFromRef);
const domSetStyleAttrFromRef = LINKS.kify(_domSetStyleAttrFromRef);
const domGetStyleAttrFromRef = LINKS.kify(_domGetStyleAttrFromRef);
const domGetPropertyFromRef = LINKS.kify(_domGetPropertyFromRef);
const domSetPropertyFromRef = LINKS.kify(_domSetPropertyFromRef);
const domSetAnchor = LINKS.kify(_domSetAnchor);
const domGetAnchor = LINKS.kify(_domGetAnchor);
...
const parentNode = LINKS.kify(_parentNode);
const firstChild = LINKS.kify(_firstChild);
const nextSibling = LINKS.kify(_nextSibling);
...
const itemChildNodes = LINKS.kify(_itemChildNodes);
const itemTextContent = LINKS.kify(_itemTextContent);
...
var clientTimeMilliseconds = LINKS.kify(_clientTimeMilliseconds);
...
function _cancel(c) {
  ...
  DEBUG.assert(TYPES.isChannel(c), "Cancelling non-channel");
...
const jsSetWindowEvent = LINKS.kify(_jsSetWindowEvent);
...
function projectDate(dt) {
    if (dt._type == "timestamp") {
        DEBUG.assert(dt._value instanceof Date);
        return dt._value;
    } else {
...
// CPS functions
const dateToInt = LINKS.kify(_dateToInt);
const intToDate = LINKS.kify(_intToDate);
const dateYear = LINKS.kify(_dateYear);
const dateMonth = LINKS.kify(_dateMonth);
const dateDay = LINKS.kify(_dateDay);
const dateHours = LINKS.kify(_dateHours);
const dateMinutes = LINKS.kify(_dateMinutes);
const dateSeconds = LINKS.kify(_dateSeconds);
const dateMilliseconds = LINKS.kify(_dateMilliseconds);
const parseDate = LINKS.kify(_parseDate);
const now = LINKS.kify(_now);
const utcOffset = LINKS.kify(_utcOffset);
const showUTC = LINKS.kify(_showUTC);
