            ...
            if (_$Types.isObject(js_parsed)) {
                if ("opcode" in js_parsed) {
                    ...
                    case "SESSION_MESSAGE_DELIVERY": {
                        ...
                        _$Session.Channel.migrateDelegatedSessions(delegated_chans, requires_lost_messages);
                    ...
                    case "GET_LOST_MESSAGES": {
                        ...
                        _$Session.Channel.handleGetLostMessages(remote_ep, ep_ids);
                        break;
                    }
                    case "DELIVER_LOST_MESSAGES": {
                        let ep_id = js_parsed.ep_id;
                        let lost_message_table = js_parsed.lost_messages;
                        _$Session.Channel.handleLostMessages(ep_id, lost_message_table);
                        break;
                    }
  ...
  return Object.freeze({
    ...
    */
    ...
    fieldVal: function (name) {
     ...
     throw ("Form element with name '" + name +"' does not exist!");
    },
...
function _stringToXml(s) {
  let res = _$List.cons({ type: 'TEXT',text: s}, _$List.nil);
  return res;
}
...
function _handleSessionException(/* List */ variables) {
  ...
  const resolvedSet = resolveSet(contSet);
...
function recv(kappa) {
  ...
  const currentPid = _$Proc.Sched.getPid();
  let msg = _$Proc.Mail.read(currentPid);
  if (msg != null) {
        ...
        return Object.freeze({
            ...
            'localAccept': function(localAPID, kappa) {
                ...
                function makeAndBlock() {
                    ...
                    _chan_blocked[our_ep] = _$Proc.Sched.getPid();
            ...
            'localRequest': function(localAPID, kappa) {
                ...
                function makeAndBlock() {
                    let new_ch = freshChannel();
                    let flipped_chan = _$Session.Channel.flip(new_ch)
            ...
            'remoteRequest': function(remote_apid, kappa) {
                // Request remotely
                _$Websocket.sendRemoteAPRequest(_$Proc.Sched.getPid(), remote_apid);
                return blockUntilAPResponse(kappa);
            },
        ...
        return Object.freeze({
            ...
            'canReceiveMessage': function(v) {
                ...
                for (let i = 0; i < containedSessions.length; i++) {
                    if (!(containedSessions[i]._sessEP2 in buffers)) {
                        return false;
                    }
                }
                return true;
            ...
            'deliverSessionMessage': function(epid, v) {
                ...
                if (!buffers[epid]) {
                    ...
                    if (blocked[epid] && _$Session.Channel.canReceiveMessage(v))
            ...
            'migrateDelegatedSessions': function(delegatedChannels, requiresLostMessages) {
                ...
                for (let i = 0; i < delegatedChannels.length; i++) {
                    ...
                    if (requiresLostMessages) pendingEndpoints[ch] = buf;
                    else buffers[ch] = buf;
                }
                return;
            },
            'handleGetLostMessages': function(remote_ep, ep_ids) {
                ...
                for (let i = 0; i < epIds.length; i++) {
                    const epId = ep_ids[i];
                    ret[epId] = lostMessages[epId];
                    lostMessages[epId] = null;
                }
            ...
            'commitChannel': function(epid, lostMessages) {
                ...
                const origBuf = pendingEndpoints[epid];
                let orphans = orphanMessages[epid];
                if (orphans == null) {
                    orphans = [];
                }
                buffers[epid] = orphans.concat(lostMessages, origBuf);
                _pendingEndpoints[epid] = null;
                _orphanMessages[epid] = null;
                return;
            },
            ...
            'prepareDelegatedChannels': function(chans) {
                ...
                for (let i = 0; i < chans.length; i++) {
                    ...
                    lostMessages[curRecvEp] = [];
                    res.unshift({ chan: curChan, buffer: curBuf});
                }
                return res;
            },
            'cancel': function(ch) {
...
function _remoteSessionSend(v, c) {
  ...
  const preparedDelegatedSessions = _$Session.Channel.prepareDelegatedChannels(delegatedSessions);
  _$Debug.debug("Prepared delegated sessions:");
  _$Debug.debug(preparedDelegatedSessions);
  return _$Websocket.sendRemoteSessionMessage(c, preparedDelegatedSessions, v);
}
...
function _send(v, c) {
  ...
  if (_$Session.Channel.isActive(peerEp)) {
...
function _exn_receive(c, cancellation_thunk, kappa) {
  ...
  if (_$Session.Channel.hasPendingMessages(localEp)
    ...
    if (_$Session.Channel.isEndpointCancelled(peer_ep)) {
...
function _jsRequestAnimationFrame(fn) {
  window.requestAnimationFrame(function () { fn(_$K.idy) });
  return;
}
...
function _jsSetOnKeyDown(node, fn) {
  ...
  node.addEventListener('keydown', function(e) { fn(e, _$K.idy) }, true);
  return;
}
...
function _jsSetOnEvent(node, event, fn, capture) {
  node.addEventListener(event, function(e) { fn(e, _$K.idy) }, capture);
  return;
}
...
function _jsSetWindowEvent(event, fn, capture) {
  window.addEventListener(event, function(e) { fn(e, _$K.idy) }, capture);
  return _$Constants.UNIT;
}
...
function _jsSetOnLoad(fn) {
  window.addEventListener('load', function(e) { fn(e, _$K.idy) }, false);
  return;
}
