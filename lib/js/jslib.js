...
function partialApply(f, x) {
    return function () {
        const env = [x].concat(Array.prototype.slice.call(arguments));
        f.env = env;
        f.apply(this, env)
    }
}
...
function _handleSessionException(affected_vars) {
  _debug("in handle session exception.");
  affected_vars.forEach(v => {
    _debug("var: " + JSON.stringify(v));
  });
}
