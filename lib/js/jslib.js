...
const _$Websocket = Object.freeze({
  ...
  onOpen: function (evt) {
    ...
    _$Websocket.status.is_connected = true;
  ...
  onClose: function (evt) {
    ...
    _$Websocket.status.is_connected = false;
    return;
  },
  ...
  try_send: function (msg) {
    ...
    if (_$Websocket.status.is_connected) {
      _socket.send(_$Links.stringify(msg));
    } else {
  ...
  return Object.freeze({
    ...
    */
    ...
    deliverMessage: function(pid, msg) {
      ...
      _wakeup(pid);
      _$Debug.debug(pid, ' now has ', _mailboxes[pid].length, ' message(s)');
      return;
    },
...
function _wrapEventHandler(handler) {
  return function (event) {
    ...
    let _cont = function () { return handler(event, _$K.idy); }
...
function _spawnWithMessages(childPid, f, messages) {
  ...
  setZeroTimeout(function () {
    ...
    f(_$K.make(function () { delete _mailboxes[childPid] }))
  });
...
function spawnWait(f, kappa) {
  ...
  return f(_$K.make(function(v) {_current_pid = parentPid; delete _mailboxes[childPid]; return _$K.apply(kappa,v);}));
}
...
function sleep(duration, kappa) {
  let current_pid = _current_pid;
  return setTimeout(function() { _current_pid = current_pid; return _$K.apply(kappa,{}); }, duration);
}
