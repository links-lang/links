...
var DEBUG = function () {
 ...
 // [IE]
 //   Node isn't defined in IE, so we need an alternative way of detecting xml nodes
 // [BUG]
...
var tilde = _continuationize(_tilde);
...
function _charlistToString(chlist) {
  DEBUG.assert(DEBUG.is_array(chlist), "_charlistToString Expected an array, got: " + chlist);
  var str = "";
  for (var i in chlist) {
    str += chlist[i]
  }
...
var stringToInt = _continuationize(_stringToInt);
var intToFloat = _continuationize(_intToFloat);
var floatToString = _continuationize(_floatToString);
var intToString = _continuationize(_intToString);
...
function _abs(f) {
  return (function () {
    ...
    var args = [];
    for (var i = 0; i < arguments.length - 1; i++) {
       args[i+1] = arguments[i]; // nb: arguments array is 0-based; our tuples are 1-based.
    }
    return f(args, kappa);
  });
}
...
function _app(f) {
  return (function (tuple, kappa) {
     ...
     for (var i = 1; i in tuple; i++) {
       args[i-1] = tuple[i]; // nb: arguments array is 0-based; our tuples are 1-based.
     }
     args[i-1] = kappa;
...
var not    = _continuationize(_not);
var empty  = _continuationize(_empty);
var hd     = _continuationize(_hd);
var tl     = _continuationize(_tl);

var length = _continuationize(_length);
var take   = _continuationize(_take);
var drop   = _continuationize(_drop);
var max    = _continuationize(_max);
var min    = _continuationize(_min);
var abs    = _abs ; // _continuationize(_abs);
var app    = _app ; // _continuationize(_app);
...
var _plus   = _continuationize(__plus);
var _times  = _continuationize(__times);
var _divide = _continuationize(__divide);
var _minus  = _continuationize(__minus);
var _hyphen = _minus;
var _star   = _times;
var _slash  = _divide;
...
var debugObj = _continuationize(_debugObj);
...
var dump = _continuationize(_dump);
...
function _getNodeById(id) {
  DEBUG.assert_noisy(DEBUG.is_array(id), "_getNodeById Expected an array, got: " + id);
...
function _domGetNodeValueFromRef(node) { return _stringToCharlist(node.value); }
var domGetNodeValueFromRef = _continuationize(_domGetNodeValueFromRef, 1)
...
function _concat (l,r) {  return l.concat(r); }
...
function _accum(fn, list, kappa) {
  DEBUG.assert(DEBUG.is_array(list), ("source for list comprehension was not a list, it was " + DEBUG.type(list))); 
...
function _singleXmlToDomNodes(xmlObj) {
     DEBUG.assert(DEBUG.is_array(xmlObj), '_singleXmlToDomNodes expected an array, but got ' + xmlObj);
    if (xmlObj[0] == "ELEMENT") {
      var tag = xmlObj[1];
      var attrs = xmlObj[2];
      var body = xmlObj[3];
      ...
      // [IE]
      //   IE doesn't allow children to be appended to a style element
      if(isElementWithTag(node, "style") && (node.styleSheet || (""+node.styleSheet == "null"))) {
        //if(_cssText)
        ...
        for (var i = 0; i < body.length; ++i) {
          if(body[i][0] == "TEXT") {
            ...
            throw "element node " + xmlObj[0] + " in style element (_singleXmlToDomNodes: IE style hack)"             
          } else {
            throw "unknown XML node " + xmlObj[0] + " in _singleXmlToDomNodes (IE style hack)"
          }
        }
        // [HACK]
      ...
      for (var name in attrs) {
         if(name == 'style' && node.style) {
           // [IE]
           //   preserve style attributes in IE
           node.style.cssText = _charlistToString(attrs['style']);
         } else {
...
function _XmlToDomNodes(xmlForest) {
  DEBUG.assert(DEBUG.is_array(xmlForest), '_XmlToDomNodes expected an array, but got ' + xmlForest);
...
//  _debug("XML constructed " + [["ELEMENT", tag, attr, [].concat.apply([],body)]]);
...
function _project(name, object) { return object[name]; }
...
//    _debug("node: "+node+"("+node.childNodes.length+")")
  ...
  _appDom(node, activate);
}

function _time () {
  return  (new Date()).getTime();
} 
var getCurrentTime = _continuationize(_time, 1);
...
function _replaceDocument(tree) {
  ...
  // [IE]
  //   hack to activate a style element in IE
...
function _invokeClientCall(_kappa, _callPackage) {
...
//  _debug("invokeClientCall: _f is " + _callPackage.__name);
  var _f = eval(_callPackage.__name);
//  alert("invokeClientCall: its value is " + _f);
//  _debug("arg to " + _callPackage.__name + ": " + _callPackage.__arg);
  _f.apply(_f, _callPackage.__args.concat(_cont));
//  _debug(_callPackage.__name + " returned " + _res);
...
//  _debug("Sending " + JSON.stringify(_res)+ " to " + _callPackage.__continuation);
...
//     _debug("Finished XmlHttpRequest");
//     _debug("serverResponse was (base64) " + request.responseText);
     // AHA:: Here's the bug; we have extra escapes in this string.
//     _debug("serverResponse was (json) " +
//            _base64decode(request.responseText.replace('\n', '')));
...
//        _debug("serverResponse was (js) " + serverResponse);
...
// search for the input value for the
  ...
  for(var i = 0; i < xs.length; ++i) {
    ...
    while(node) {
      if(node == containingForm) {
         return _stringToCharlist(xs[i].value);
      }
      node = node.parentNode;
    }
  }

  DEBUG.assert(false, "Form element with name '" + name +"' does not exist!");
}
...
function recv(kappa) {
DEBUG.assert(arguments.length == 1, ('recv received '+arguments.length+ ' arguments, expected 1'));
...
function _yield(f, args, k) {
  DEBUG.assert_noisy(DEBUG.is_function(k), "_yield: expected a function, got: " + k);

  DEBUG.assert(DEBUG.is_array(args), "_yield: expected an array, got: " + args);
//  var arguments = args.concat([k]);
  var argslen = args.length;
  var arguments = ___append(args, k);
//  DEBUG.assert(argslen + 1 == arguments.length, 'arglen check 1');
  ++_yieldCount;
...
//      _debug("yielding");
      var current_pid = _current_pid;
      setTimeout((function() {
//                      DEBUG.assert(argslen + 1 == arguments.length,  'arglen check 2');
...
//                      f(a, k)}),
      ...
      throw new _Continuation(function () { return f.apply(f, ___append(args, k)); });
    }
  } 
  else {
return f.apply(f, ___append(args, k));
...
function _yieldCont(k, a) {
  DEBUG.assert_noisy(DEBUG.is_function(k), "_yieldCont: expected a function, got: " + k);
  ++_yieldCount;
//  _debug("yield count: "^_yieldCount);
  if ((_yieldCount % _yieldGranularity) == 0) {
    if (!_handlingEvent) {
...
function attribute(xml, attr, kappa) {
...
function textContent (node, kappa) {
  try { return kappa(node.innerHTML) } 
  catch (e) { return kappa("") }
}
...
function _makeUrlArgs() {
  ...
  for (var i = 0; i < arguments.length; i++) {
    ...
    DEBUG.assert(typeof(arguments[i][1]) != 'function',
                 "Cannot marshal function value (" + arguments[i][0] + ") to send to the server");
...
function _jsStrConcat(s1, s2) { return s1 + s2; }
