        ...
        if(key != null) {
          ...
          for(var lAttrName in hs) {
            ...
            jslib.event.addListener(listenerElem, eventName,
              function (key, name){
                return function (e) {
		  //_debug("firing event: "+name+ " on elem with key=" + key);
...
var debug = LINKS.kify(_debugChl);     // This is undesirable:
...
function _domAppendChildRef(appendNode, parentNode) {
  try {
    ...
    // See http://developer.mozilla.org/en/docs/Gecko_1.9_Changes_affecting_websites
    // Should probably be more precise as to WHAT the error is.
    var lNewNode = parentNode.ownerDocument.adoptNode(appendNode);
...
function _domSetNodeValueFromRef(node, val) {
  node.value = $str(val);
}
...
var domSetNodeValueFromRef = LINKS.kify(_domSetNodeValueFromRef);
...
var domSetWinDocAttribute  = LINKS.kify(_domSetWinDocAttribute);
...
// String -> [domRef]
function _getElementsByName(name) {
  return _collectionToArray(document.getElementsByName($str(name)));
}
...
// Adopt node from a foreign document
// (domRef, domRef) -> domRef
function _adoptNode(node, doc) {
  return doc.adoptNode(node);
}

var getElementsByTagName = LINKS.kify(_getElementsByTagName);
var getElementsByTagNameFromRef = LINKS.kify(_getElementsByTagNameFromRef);
var getElementsByName = LINKS.kify(_getElementsByName);
var getContentDocument = LINKS.kify(_getContentDocument);
...
var getMouseButton = LINKS.kify(_getMouseButton)
var getModifiers = LINKS.kify(_getModifiers)
var stopPropagation = LINKS.kify(_stopPropagation);
var adoptNode = LINKS.kify(_adoptNode);

function _split(pattern, string) {
  var lString = $str(string);
  var lRegEx = new RegExp($str(pattern));

  return (lString.split(lRegEx)).map($chl);
}

var split = LINKS.kify(_split);

function _replace(pattern, src, dst) {
  var lString = $str(src);
  var lRegEx = new RegExp($str(pattern));

  return $chl(lString.replace(lRegEx, ($str(dst))));
}

var replace = LINKS.kify(_replace);
...
var getCurrentTime = LINKS.kify(_time);
...
function sleep(duration, kappa) {
  var current_pid = _current_pid;
  setTimeout(function() { _current_pid = current_pid; kappa({}); }, 
             (duration * 1000));
}
