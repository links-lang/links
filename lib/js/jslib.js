...
const LINKS = new (function() {
  ...
  function resolveServerValue (state, obj) {
    ...
    switch (obj._tag) {
      default:
        throw new Error("Mistagged server value \"" + JSON.stringify(obj) + "\"");
      ...
      case "ClientFunction":
        const f = (!TYPES.isObject(obj.environment)) ?
          eval(obj.func) :
          partialApply(eval(obj.func), resolveServerValue(obj.environment));
      ...
      case "List":
        if (obj._head === null || obj._head === undefined) {
          return LINKEDLIST.Nil; // check
        }

        return LINKEDLIST.Cons(
          resolveServerValue(state, obj._head),
          resolveServerValue(state, obj._tail)
        )
