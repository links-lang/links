...
var LINKS = function() {
  ...
  function resolveServerValue(_obj) {
    if (_obj instanceof Object) {
      for (var i in _obj) {
         resolveServerValue(_obj[i]);
         if (_obj[i].func) {
...
//           f.environment = _obj[i].environment;
           ...
           resolveServerValue(hs);
           delete _obj[i].eventHandlers;
           _obj[i].key = _registerEventHandlers(hs);
         } else if (_obj[i].pid) {
           // TODO: maintain a mapping between server and client pids?
           var serverPid = _obj[i].pid;
           var process = _obj[i].process;
           var messages = _obj[i].messages;

// {pid=1, process={func=_1234}, messages=[]}

           // spawn the client-side process
           var pid = _spawnWithMessages(messages, process);
...
// >>>>>>> sessions
          ...
          _invokeClientCall(kappa, serverResponse, synch);
...
function _spawnWithMessages(messages, f) {
  _maxPid++;
  var childPid = _maxPid;
  // is it really necessary to check whether _mailboxes[pid] is already defined?
  if (!_mailboxes[childPid])
