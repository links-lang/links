...
function _handleSessionException(cont_variables, return_variables) {
  ...
  affected_channels.forEach(v => {
    _debug("channel: " + JSON.stringify(v));
  });
}
...
function _cancel(c) {
  ...
  DEBUG.assert(_sessEP1 in c, "Cancelling non-channel");

  const peer_ep = c._sessEP1;
  const local_ep = c._sessEP2;

  function cancelContainedValues() {
    buf_set = new Set(_buffers[local_ep]);
  ...
  function notifyPeer() {
    if (peer_up in _buffers) {
      _wakeup(_chan_blocked[sendEP]);
      delete _chan_blocked[sendEP];
    } else {
      DEBUG.assert(false, "Remote cancellation not yet supported.");
    }
  }
