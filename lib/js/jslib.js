...
const LINKS = new (function() {
  ...
  function _activateJsonState(state, clientId, processes, handlers, aps, buffers) {
    ...
    for (let i in handlers) {
      var h = handlers[i];
      var hs = resolveServerValue(state, eval(h.eventHandlers));
    ...
    for (let i in processes) {
      var p = resolveServerValue(state, processes[i]);
  ...
  function resolveMobileState(state, processes, handlers) {
    ...
    for (var i in handlers) {
      ...
      hs = resolveServerValue(state, hs);
      _registerMobileEventHandlers(h.clientKey, hs);
    }
  ...
  function resolveServerValue (state, obj) {
    if (obj instanceof Object === false) {
      // Non-Object? return as it is
      return obj;
    }

    if (obj.type || LINKEDLIST.isNil(obj)) {
      // XML or nil? skip.
      return null;
    }

    // TODO: how to resolve Variant, List and REcord?
    // TODO: Do all objects include a tag?
    // TODO: Same treatment for FunctionPtr and ClientFunction?
    switch (obj._tag) {
      // Primatives
      case "Bool":
      case "Int":
      ...
      case "ClientFunction":
        const f = (!TYPES.isObject(obj.environment)) ?
          eval(obj.func) :
          partialApply(eval(obj.func), eval(obj.environment));
      ...
      case "Record":
        ...
        delete obj._tag;
        for (let k in obj) {
          newObj[k] = resolveServerValue(state, obj[k]);
        }
        return newObj;
      case "Variant":
        var value = {};
        var newObj = { '_label': obj._label, '_value': value};

        for (let k in obj._value) {
          value[k] = resolveServerValue(state, obj[k]);
        }
        return newObj;
      ...
      case "List":
        ...
        return { '_head': resolveServerValue(obj._head),
                 '_tail': resolveServerValue(obj._tail) }
      ...
      case "ServerSpawnLoc":
      case "ClientupawnLoc":
        // TODO: should they return a new object?
        delete obj._tag;
        return obj;
  ...
  function remoteCallHandler (kappa, request) {
    return function () {
      if (request.readyState == CONSTANTS.AJAX.XHR_STATUS_IS_COMPLETE && !request.finished) {
        ...
        const box = resolveServerValue(state, { content: serverResponse.content.value });
