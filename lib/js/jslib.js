...
const _$AP = (function() {
    ...
    let returned_channels = {}; // Channels that have been returned
    ...
    function blockUntilAPResponse(kappa) {
        ...
        return _$Proc.block(
            ...
            function() {
                ...
                // and continue
                _$Debug.assert(_returned_channels[currentPid] != undefined,
                               "resuming process after remote accept, but no channel available!");
                const chan = _returned_channels[currentPid];
                delete _returned_channels[currentPid];
    ...
    return Object.freeze({
        ...
        'localRequest': function(localAPID, kappa) {
            ...
            case ACCEPTING:
                ...
                return _$K.apply(kappa, makeFlippedChan(top));
            default:
                throw ("Invalid access point state: " + ap.state);
            }
        },
...
const _$Websocket = Object.freeze({
  buffer : [],
  state: {
      socket: null,
      is_connected: false
  ...
  connect: function (ws_uri) {
    ...
    _$Websocket.state.socket = new WebSocket(ws_uri);

    /* Set up all of the event handlers */
    _$Websocket.state.socket.onopen = function (evt) {
       _$Websocket.onOpen(evt);
       return;
    };

    _$Websocket.state.socket.onclose = function (evt) {
       _$Websocket.onClose(evt);
       return
    };

    _$Websocket.state.socket.onerror = function (evt) {
       _$Websocket.onError(evt);
       return;
    };

    _$Websocket.state.socket.onmessage = function (evt) {
  ...
  onOpen: function (evt) {
    ...
    _$Websocket.state.is_connected = true;
  ...
  onClose: function (evt) {
    ...
    _$Websocket.state.is_connected = false;
    return;
  },
  ...
  serialise_and_send: function (msg) {
    _$Websocket.state.socket.send(_$Links.stringify(msg));
    return;
  },
  ...
  try_send: function (msg) {
    ...
    if (_$Websocket.state.is_connected) {
      _$Websocket.state.socket.send(_$Links.stringify(msg));
    } else {
...
function recv(kappa) {
  ...
  const currentPid = _$Proc.Sched.getPid();
  let msg = undefined;
  if (msg = _$Proc.Mail.read(currentPid)) {
...
function accept(ap, kappa) {
  ...
  if (_is_valid_server_ap(ap)) {
    return _$AP.remoteAccept(get_server_apid_from_ap(ap), kappa);
    ...
    if (get_client_id_from_ap(ap) != _client_id) {
      ...
      return _$AP.localAccept(get_apid_from_ap(ap), kappa);
    }
  } else {
...
function request(ap, kappa) {
  if (_is_valid_server_ap(ap)) {
    return _$AP.remoteRequest(get_server_apid_from_ap(ap), kappa);
  } else if (_is_valid_client_ap(ap)) {
    if (get_client_id_from_ap(ap) != _client_id) {
      _$Debug.assert(false, "alas, requesting from a remote client AP is not yet supported");
      return;
    } else {
      return _$AP.localRequest(get_apid_from_ap(ap), kappa);
    }
  } else {
...
function _default_receive(c, kappa) {
  ...
  if (buf_len > 0 && canReceiveMessage(_buffers[local_ep][buf_len - 1])) {
    ...
    return _$K.apply(kappa, {1:msg, 2:c});
      ...
      const currentPid = _$Proc.Sched.getPid();
    _chan_blocked[local_ep] = currentPid;
  ...
  if (buf_len > 0 && canReceiveMessage(_buffers[local_ep][buf_len - 1])) {
    ...
    if (isEndpointCancelled(peer_ep)) {
      ...
      _$Proc.block(currentPid,
        function () {
          _$Proc.Sched.setPid(currentPid);
          return receive(c, cancellation_thunk, kappa);
        });
    }
  }
}
