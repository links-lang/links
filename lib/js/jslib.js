...
var WEBSOCKET = function() {
  return {
    ...
    onMessage : function(evt) {
      ...
      if (DEBUG.is_object(evt.data)) {
        if ("opcode" in evt.data) {
          switch (evt.data.opcode) {
            case "MESSAGE_DELIVERY":
              var local_pid = evt.dest_pid;
              var msg = evt.val;
    ...
    sendRemoteClientMessage : function(destClientId, destPid, msg) {
      ...
      _socket.send(JSON.stringify(to_send_json));
    },

    sendRemoteServerMessage : function(destServerPid, msg) {
      _debug("Sending message bound for server process " + destPid + ", msg: "
             + JSON.stringify(msg));
...
//    _debug('arguments: ' + LINKS.stringify(_callPackage.__args).str);
  ...
  function deliverMessage(pid, msg) {
    if (!_mailboxes[pid]) {
      _makeMailbox(pid);
    }
...
//  _dump(_mailboxes)
  ...
  if (valid_server_pid) {
    ...
    WEBSOCKET.sendRemoteServerMessage(serverPid._serverPid, msg);
    ...
    if (get_client_id(pid) == _client_id) {
      ...
      // Remote send
      WEBSOCKET.sendRemoteClientMessage(get_client_id(pid), msg);
    }
  }
  //_dumpSchedStatus();
  return;
