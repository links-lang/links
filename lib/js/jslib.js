...
var LINKS = function() {
  ...
  function _resolveJsonState(state, processes, handlers) {
    ...
    for (var i in processes) {
      var p = processes[i];
      p.pid = _registerMobileProcess(state, p.pid);
    }
  ...
  function _activateJsonState(state, client_id, processes, handlers) {
    // set client ID
...
//           f.environment = _obj[i].environment;
         ...
         // SJF: Right, I think this is the only place that the old "pid" field was
         // used. From what I can tell, it's just a straight replacement of the pid
         // placeholder with the local pid on the client. Example:
         // Given obj[i] = { pid : 1 }, with a mapping { 1 |-> 3 }, we would then get
         // obj[i] = 3.
         // Of course, this can't stand -- firstly, pid is no longer a field that is sent!
         // Secondly, we need to retain the richer PID information -- we could have either
         // client *or* server PIDs.
         // I don't think we'll actually need _lookupMobilePid (at least not in webserver mode)
         // since now we have client IDs as well as client PIDs, they should be the same on both
         // sides? I'll keep it for now, and play with it a bit more later.
         /*
         } else if (_obj[i].pid) {
           _obj[i] = _lookupMobilePid(state, _obj[i].pid);
         }
         */
         } else if (_obj[i]._clientPid) {
           // TODO: Do we need this resolution still?
           _obj[i] = { _clientPid : _lookupMobilePid(state, _obj[i]._clientPid), _clientId : _obj[i]._clientId };
         } else if (_obj[i]._serverPid) {
           // TODO: I don't think we actually need to do anything with this?
         }
      }
    }
  }
...
//    _debug('arguments: ' + LINKS.stringify(_callPackage.__args).str);
  ...
  var LINKS = {
    ...
    activateJsonState : function (state, s) {
      _activateJsonState(state, s.client_id, s.processes, s.handlers);
    },
...
//  _dump(_mailboxes)
  ...
  if (valid_server_pid) {
    // TODO: Implement client -> server send logic here.
    DEBUG.assert(false, "Cannot currently send from client PID to server (soon, I promise!)");
  } else if (valid_client_pid) {
    // TODO: Implement client -> client send logic here.
    DEBUG.assert(get_client_id(pid) == _client_id, "Cannot currently send from client to client (soon, I promise!)")
    _debug("sending message '" + msg._label + "' to pid " + pid._clientPid);
    var client_pid = pid._clientPid;
    if (!_mailboxes[client_pid])
      _makeMailbox(client_pid);
    _mailboxes[client_pid].unshift(msg);
    _wakeup(client_pid);
    _debug(client_pid + ' now has ' + _mailboxes[client_pid].length + ' message(s)');
  }
  //_dumpSchedStatus();
  return;
}
...
function accept(ap, kappa) {
  // HACK: allows us to use access points that were created
  // on the server
  if(_aps[ap] == undefined) {
    _aps[ap] = {accepters: [], requesters: []};
    _nextAP = Math.max(_nextAP, ap+1);
  }
