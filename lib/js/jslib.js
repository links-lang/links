...
var LINKS = function() {
  ...
  var _invokeClientCall = function (_kappa, _callPackage) {
    // _debug("invoking client call.");
    var _res = null;
    var _cont = function(r) {
      // [HACK]
    ...
    // NOTE: there is a problem in that variables defined within this function
    // can shadow a user-defined function named in __name.
    var _f = eval(_callPackage.__name);
    _f.apply(_f, _callPackage.__args.concat(_cont));
  ...
  var LINKS = {
    ...
    remoteCall : function(kappa) {
     return function(name, arguments) {
       ...
       // make sure the correct pid is set for kappa!
       var safe_kappa = function (response) {
       ...
       request.setRequestHeader('Content-Type',
                                'application/x-www-form-urlencoded');
       request.onreadystatechange = _remoteCallHandler(safe_kappa, request);
   
       request.send("__name=" + _base64encode(name) + 
                    "&__args=" + _base64encode(JSON.stringify(arguments)));
       if (in_main_process) {
         return _remoteCallHandler(safe_kappa, request)();
       }
     }
    },

    /// Client calls
    invokeClientCall : function(kappa, serverResponse) {
      return _invokeClientCall(kappa, serverResponse);
    },
  ...
  LINKS.accumAux = function (fn, list, result, kappa) {
      if (LINKS.isEmpty(list)) kappa(result)
      else {
        h = list[0];
        _yield(fn, [h],
               function(himg) {
                 t = list.slice(1);
                 LINKS.accumAux(fn, t, result.concat(himg), kappa)
               } );
      }
    };
...
function _continuationize() {
  alert("_continuationize is deprecated, use LINKS.kify instead.");
  return LINKS.kify.apply(LINKS.kify, arguments);
}
...
var debug = LINKS.kify(_debug, 1);

_alert = _debug;               // alert's not defined in Links; do we need this?
...
var alertDialog = LINKS.kify(_alertDialog, 1);
...
var debugObj = LINKS.kify(_debugObj, 1);
...
var negate = LINKS.kify(_negate, 1);
...
function _insertBefore(insertXml, beforeNode) {
  ...
  var nodes = (LINKS.XmlToDomNodes(insertXml)).reverse();
...
//replaceDocument : xml -> ()
var replaceDocument = LINKS.kify(_replaceDocument, 1);
...
var insertBefore = LINKS.kify(_insertBefore, 2);
var appendChildren = LINKS.kify(_appendChildren, 2);
var replaceNode = LINKS.kify(_replaceNode, 2);

var domInsertBeforeRef = LINKS.kify(_domInsertBeforeRef, 2);
var domAppendChildRef = LINKS.kify(_domAppendChildRef, 2);
var removeNode = LINKS.kify(_removeNode, 1);

var getDocumentNode = LINKS.kify(_getDocumentNode, 1);
var getNodeById = LINKS.kify(_getNodeById, 1);
var isNull = LINKS.kify(_isNull, 1);
...
var getInputValue = LINKS.kify(_getInputValue, 1);
...
getValue = LINKS.kify(_getValue, 1);
...
function _domSetStyleAttrFromRef(nodeRef, attr, value) {
  return $chl(nodeRef.style[$str(attr)] = $str(value));
}
...
var domGetNodeValueFromRef = LINKS.kify(_domGetNodeValueFromRef, 1)

var domGetTagNameFromRef = LINKS.kify(_domGetTagNameFromRef, 1);
var domGetAttributeFromRef = LINKS.kify(_domGetAttributeFromRef, 2);
var domSetAttributeFromRef = LINKS.kify(_domSetAttributeFromRef, 2);
var domSetStyleAttrFromRef = LINKS.kify(_domSetStyleAttrFromRef, 2);
var domGetStyleAttrFromRef = LINKS.kify(_domGetStyleAttrFromRef, 2);
...
var parentNode = LINKS.kify(_parentNode, 1);
var firstChild = LINKS.kify(_firstChild, 1);
var nextSibling = LINKS.kify(_nextSibling, 1);
...
var swapNodes = LINKS.kify(_swapNodes, 2);
...
var replaceChildren = LINKS.kify(_replaceChildren, 2);
...
// Use Yahoo's library only if the event isn't a keyup/down.
// This is necessary due to getCharCode only returning a value
// when used with keypress unless charCode is set.
function _getCharCode(event) { 
  if((event.type == "keyup") || (event.type == "keydown"))
    return event.keyCode; 
  else 
    return YAHOO.util.Event.getCharCode(event);
}

// Get a list of the modifiers used.
function _getModifiers(event) {
  var lModifs = [];
  
  if (event.type.match(/^key|click$|mouse[^o]/))
  {
    if (event.shiftKey)
    {
      lModifs.push($chl("16"));
  ...
  {
    throw("Cannot get modifiers for event of type " + event.type);
  }
}

// Get the mouse button used on a click. 
// Why doesn't this exist in the Yahoo lib?
// Event -> Int
function _getMouseButton(event) {
  // There is probably a discrepancy between IE and FF.
  return event.button;
}
...
function _getElementsByTagName(name) {
  var name = $str(name);
  //_debug("Name is: " + name);
  //_debug("Element: " + document.getElementsByTagName(name)[0].id);
  return document.getElementsByTagName(name);
}

// String -> domRef -> [domRef]
function _getElementsByTagNameFromRef(name, ref) {
  var name = $str(name);
  return ref.getElementsByTagName(name);
}

// domRef -> domRef
function _getContentDocument(node) {
  //_debug(node.contentDocument.URL);
...
function _xmlToString(obj) {
  var domObj = LINKS.XmlToDomNodes(obj);
  var serializer = new XMLSerializer();
  var outString = "";

  _debug("Len: " + domObj.length);
  _debug("Serialized: " + serializer.serializeToString(domObj[0]));
  for (var i = 0; i < domObj.length; i++)
  {
    outString = outString + serializer.serializeToString(domObj[i]);
  }
...
// domRef -> ()
function _execBold(targetDoc) {
  targetDoc.execCommand("bold", false, null)
}

function _execInsertImage(document, url) {
  document.execCommand("insertImage", false, charlistToString(url))
}

var getElementsByTagName = LINKS.kify(_getElementsByTagName, 1);
var getElementsByTagNameFromRef = LINKS.kify(_getElementsByTagNameFromRef, 2);
var getContentDocument = LINKS.kify(_getContentDocument, 1);
var getInnerHtml = LINKS.kify(_getInnerHtml, 1);
var execBold = LINKS.kify(_execBold, 1)
var execInsertImage = LINKS.kify(_execInsertImage, 2)

var getTarget = LINKS.kify(_getTarget, 1)
var getTargetValue = LINKS.kify(_getTargetValue, 1)
var getPageX = LINKS.kify(_getPageX, 1)
var getPageY =LINKS.kify(_getPageY, 1)
var getFromElement = LINKS.kify(_getFromElement, 1)
var getToElement = LINKS.kify(_getToElement, 1)
var getTime = LINKS.kify(_getTime, 1)
var getCharCode = LINKS.kify(_getCharCode, 1)
var getMouseButton = LINKS.kify(_getMouseButton, 1)
var getModifiers = LINKS.kify(_getModifiers, 1)
var stopPropagation = LINKS.kify(_stopPropagation, 1);
...
var stringifyB64 = LINKS.kify(_stringifyB64, 1);
...
var stringToXml = LINKS.kify(_stringToXml, 1);
var intToXml = LINKS.kify(_intToXml, 1);

// not in library.ml yet
var floatToXml = LINKS.kify(_floatToXml, 1);
...
getTagName = LINKS.kify(_getTagName, 1);
getAttributes = LINKS.kify(_getAttributes,1 );
hasAttribute = LINKS.kify(_hasAttribute, 2);
getAttribute = LINKS.kify(_getAttribute, 2);
getChildNodes = LINKS.kify(_getChildNodes, 1);
getTextContent = LINKS.kify(_getTextContent, 1);
...
// DomNode -> bool
var isElementNode = LINKS.kify(_isElementNode, 1);
...
var getCurrentTime = LINKS.kify(_time, 1);
...
function spawnWait(f, kappa) {
  ...
  f(function(v) {_current_pid = parentPid; kappa(v)});
}
...
function _yieldCont(k, a) {
  ...
  if ((_yieldCount % _yieldGranularity) == 0) {
    if (!_handlingEvent) {
      ...
      throw new _Continuation(function () { k(a) });
    }
  } 
  else {
    return k(a);
...
var attribute = LINKS.kify(attribute, 2);
...
var objectType = LINKS.kify(_objectType, 1);
...
var toUpper = LINKS.kify(_toUpper, 1);
var toLower = LINKS.kify(_toLower, 1);
...
function _chartest(r) { 
    return LINKS.kify(function (c) { return r.test(c) }, 1);
}
...
var chr = LINKS.kify(_chr, 1);
function _ord(c) { return c.charCodeAt(0); }
var ord = LINKS.kify(_ord, 1);
...
var _sqrt = Math.sqrt; var sqrt = LINKS.kify(_sqrt, 1);
var _floor = Math.floor; var floor = LINKS.kify(_floor, 1);
var _ceiling = Math.ceil; var ceiling = LINKS.kify(_ceiling, 1);
var _tan = Math.tan; var tan = LINKS.kify(_tan, 1);
var _sin = Math.sin; var sin = LINKS.kify(_sin, 1);
var _cos = Math.cos; var cos = LINKS.kify(_cos, 1);
var _log = Math.log; var log = LINKS.kify(_log, 1);

function _environment() {
  LINKS.unimpl("environment");
}
var environment = LINKS.kify(_environment, 1);
...
var setCookie = LINKS.kify(_setCookie, 2);

function _getCookie(cookieName) {
  var c = QUIRKS.readCookie($str(cookieName))
  if(!c) c = "";
  return $chl(c)
//  return ""
 // return $chl(QUIRKS.readCookie($str(cookieName)));
}
var getCookie = LINKS.kify(_getCookie, 1);
