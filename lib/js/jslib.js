...
var LINKS = function() {
  ...
  function resolveFunctions(_obj, _closureTable) {
    if (_obj instanceof Object)
      for (var i in _obj) {
         if (_obj[i].function) {
           _debug("resolving " + _obj[i].function);
           _debug(eval(_obj[i].function));
...
_debug("Note: environmentless function resolved");
             f = eval(_obj[i].function)({});
           } else
             f = eval(_obj[i].function)(eval(_obj[i].environment));
           f.location = _obj[i].location; // This may be set to 'server' by the server serializer.
           f.function = _obj[i].function;
...
function _domGetAttributeFromRef(nodeRef, attr) {
  var attr = $str(attr);
  _debug("node[attr]: " + $chl(nodeRef.getAttribute(attr)));
  if (attr == 'offsetTop') {
