...
var _dwindow = DEBUGGING ? open('', 'debugwindow',
                                'width=550,height=800,toolbar=0,scrollbars=yes') 
                         : null;
...
var DEBUG = function () {
  ...
  assert_noisy : function(value, message) {
    if (value != true) {
      try {  
        ...
        var msg = "<b>ASSERTION FAILED!</b> " + message + " in function <pre>" 
                    + arguments.callee.caller + "</pre><b>Stack</b>: <pre>" 
                    + e.stack+"</pre>";
        _debug(msg);
  ...
  is_xmlnode : is_xmlnode,              // FIXME should be is_domref or similar
...
var LINKS = function() {
  ...
  var LINKS = {
    ...
    XML : function (tag, attrs, body, kappa) {
      kappa([["ELEMENT", tag, attrs, [].concat.apply([],body)]])
    },
    ...
    activateHandlers : function (node) {
      ...
      function activate(node) {
        ...
        _debug("activating events for: " + key + " ("+node+", "+node.childNodes.length+" subnodes)")
        if(key != null) {
...
// _debug : String u Charlist -> ()
//   It handles String or Charlist equally well, as they are disjoint.
function _debug(msg) {
   if (DEBUGGING) {
     var msg = DEBUG.is_array(msg) ? $str(msg) : msg;
...
function _attachEventHandlers(xml, actions) {
   _debug("attaching " + actions.length + " handlers to " + xml);
//   DEBUG.assert(_isXml(xml),// && xml.length == 1,
//                "attachEventHandlers expects single Xml item");
   var xml = xml[0]; // get the XmlItem inside the Xml container
   _debug("in attachEventHandlers, got: " + xml);
   var key = _xmlAttribute(xml, 'key');
   DEBUG.assert(key != None,
                "no _key attribute on object in attachEventHandlers");
   ...
   return [xml]; // return the XmlItem to its Xml container
}
var attachEventHandlers = LINKS.kify(_attachEventHandlers, 2);
...
function _attribute(nodeRef, attr) {
...
//     return ({_label:'None', '_value':({})}); 
  ...
  var val = nodeRef[attr];

  if (val == undefined) //return({_label:'None', '_value':({})});
    return None
  else
    return Some(val)
   // return({'_label':'Some', '_value':val});
}
var attribute = LINKS.kify(attribute, 2);

function _xmlAttribute(xml, attr) {
  DEBUG.assert (xml[0] == 'ELEMENT', "xmlAttribute expected Xml element node");
...
//  var attr = $str(attr);
  ...
  for (var name in attrs) {
    _debug("examining attribute " + name + ": " + attrs[name]);
    if (name == attr) {
      return Some(attrs[name]);
    }
  }

  return None;
}
var xmlAttribute = LINKS.kify(xmlAttribute, 2);
...
var _toLower = String.prototype.toLowerCase;
var toLower = LINKS.kify(_toLower, 1);
