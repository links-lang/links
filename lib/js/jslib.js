...
var WEBSOCKET = function() {
  return {
        ...
        if ("opcode" in js_parsed) {
          switch (js_parsed.opcode) {
            ...
            case "SESSION_MESSAGE_DELIVERY":
              ...
              migrateDelegatedSessions(delegated_chans);
    ...
    sendLostMessages : function (ep_id, msgs) {
      _debug("Delivering lost messages for EP " + ep_id);
      var to_send_json =
        { opcode: "LOST_MESSAGES", epID: ep_id, msgs: msgs };
      WEBSOCKET.try_send(to_send_json);
    }
  }
...
// Adds the buffers of a set of delegated channels
function migrateDelegatedSessions(delegated_chans) {
  for (var i = 0; i < delegated_chans.length; i++) {
    var deleg_chan = delegated_chans[i];
    buffers[deleg_chan.ep_id] = deleg_chan.buf;
  }
}
...
//   [ { chan : _sessEP2, buffer : <list of values in the buffer for recv_port> } ]
//
// Side-effects:
...
function prepareDelegatedChannels(chans) {
  ...
  for (var i = 0; i < chans.length; i++) {
    ...
    // _delegating[cur_recv_ep] = [];
    res.unshift({ chan: cur_recv_ep, buffer: cur_buf});
  }
  return res;
}
...
function _remoteSessionSend(v, c) {
  ...
  _debug("Delegated sessions:");
  _debug(JSON.stringify(delegated_sessions));
