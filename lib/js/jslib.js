...
function _new() {
  // console.log("Creating new access point " + _nextChannel);
  _channels[_nextChannel] = [{proc: 0, messages: []}, {proc: 0, messages: []}]
  _nextChannel = _nextChannel + 1;
  return _nextChannel - 1;
}

function _accept(ap) {
  // console.log("Accept called on AP " + ap);
  _channels[ap][0] = {proc: _current_pid, messages: []};
  return { channel: ap, direction: 0 };
}

function _request(ap) {
  // console.log("Request called on AP " + ap);
  _channels[ap][1] = {proc: _current_pid, messages: []};
  return { channel: ap, direction: 1};
}
