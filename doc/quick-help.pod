...
A table can be coerced to a list using the C<asList> function:

  asList : TableHandle(r, w) -> [r]
...
of C<t> at compile time. This is a limitation that will be fixed in
the next release of Links.)
...
C<z <-- t> is syntactic sugar for C<z <- asList(t)>. So with the above
...
Tables now have preliminary support for constraints. Currently the
only constraint that can be applied is to define some fields as
readonly. To assign to the variable C<t> the table "people" with
fields C<id> of type Int and C<name> of type String where C<id> is
readonly, write:

  var t = table "people" with (id:Int, name:String) where id readonly from db;

The variable C<t> has type C<TableHandle((id:Int, name:String),
(name:String))>, as the C<id> field is C<readonly>.
...
It is also possible to define parameterised type aliases.
  ...
  typename T(a,b) = ((a -> b), [a]) -> [b]

defines the parameterised type alias T(a), and you can write:
...
=item *
fold_left1 : ((a, a) -> a, [a]) -> a

=item *
fold_right1 : ((a, a) -> b, [a]) -> a
...
C<sendSuspend(xmlFunc)> calls the function C<xmlFunc> to generate an
XML document and exits the program using that XML document as the
output.

C<sendSuspend> captures the current continuation when it is called and
passes this as the argument to C<xmlFunc>. Thus, C<xmlFunc> can use
its argument to resume computation at the point where C<sendSuspend>.

See C<examples/login-flow.links> and C<examples/wine.links> for examples of
the use of C<sendSuspend>.

=back

=head2 Formlet library

hese routines can be used with the C<formlet>/C<yields> syntax
...
=item *
plug : (Xml -> Xml) -> Formlet (a) -> Formlet (a)
...
=item *
hidden : String -> Formlet (String)

Constructs a formlet containing a hidden field whose value is given in
the argument.
...
=item *
submitButton : (String,a) -> Formlet (Maybe(a))

=item *
resetButton : (String,a) -> Formlet (Maybe(a))

=item *
pushButton : (String,a) -> Formlet (Maybe(a))

=item *
button : (String,[|Submit|Reset|Button|],a) -> Formlet (Maybe(a))

C<button("Text", behavior, key)> constructs a simple button element
that will yield C<Just key> if this button is the one that submits the
form and C<Nothing> otherwise. C<behavior> choose whether the button
is an HTML submit button, a reset button, or an undistinguished button
(see the HTML spec).
...
=item *
inputRadiogroup : ([(String, a)], (String, a), [(String,a)], [(Xml, a)] -> Xml) -> Formlet (String) 
...
=item *
choices : [(a,String)] -> Formlet ([a])

Constructs a formlet containing a multiple-select scrollbox containing
the options listed in the argument, as per L<choice> above.

=item *
render : (Formlet (a), a -> Xml) -> Xml

Given a formlet and a submission-handler function, returns the XML
that realizes the formlet (a C<E<lt>formE<gt>> element) and whose
C<action> will trigger the handler function. The handler function
will be invoked at the server and will represent a page-transition.

=item *
renderMultipart : (Formlet (a), a -> Xml) -> Xml
