...
TBD: make this example generic instead of forward-referencing tables.
...
L</"Database Access">), describing models of an automobile make. Then the
...
database C<factorials> using the C<mysql> driver, using the default
...
For example, to create a handle to the table
C<factorials> with fields C<i> and C<f> both of type C<Int> associated
with the database C<db>, and bind it to the variable C<fac_table>, use:

  var fac_table = table "factorials" with (i : Int, f : Int) from db;

The variable C<fac_table> then has type C<TableHandle((i:Int,
f:Int),(i:Int, f:Int))>.  The first argument to C<TableHandle> is the
type used when reading from the table and the second is the type for
writing. In this instance, the two are the same, so it is not
necessary to list them separately. (TBD: how do I describe them if I
want to?)
...
=head3 Table special features: readonly and default
...
(TBD: move insert/update before this instead of forward-referencing.)

A read-only field must not be included in an insert or an update. A
defaulted field need not be included in an insert.
...
and C<nationality>, where the C<id> field cannot be modified from the
Links code, and C<nationality> has a default value so it is not
necessary to supply one when inserting rows. (TBD: Is giving a value
optional or is it forbidden?)

The components of the type assigned by links to a table handle
indicate the I<readable> fields, the I<writeable> fields and the
...
readonly. Only C<name> is I<needed> when inserting, as C<nationality>
has a default and C<id> is readonly.
...
Links cannot translate to SQL in the context of the above query. If so,
an error is generated, usually at the call site, explaining that the 
function passed was not "tame"--the Links term for a function that 
...
As explained above, a links program can read from a database using
table-handles and C<for>-comprehensions.

Database modifications--insert, update and delete operations--are also
