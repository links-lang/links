...
To introduce a forest of XML nodes that have no mutually enclosing XML
element, use the special C<E<lt>#E<gt>...E<lt>/#E<gt>> syntax:

    <#>
      Name: <b>{name}</b>
    </#>

Here a sequence of an XML text node and a B element are included as a
single expression. This value can be included directly in other XML
quasis:
    ...
    <div>
      {nameLine} <br />
      (c) 2006 Links Team, University of Edinburgh
    </div>
