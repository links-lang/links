...
directly in other XML quasis:
    ...
    <div>
      {nameLine} <br />
      &copy; 2005-2009 Links Team, University of Edinburgh
    </div>
...
Unfortunately, the C<l:> mechanism breaks abstraction. To address this
issue Links version 0.4 has introduced L</"Formlets">, a mechanism for
abstracting fragments of forms.
...
=head3 Multiple generators

It is possible to use a for comprehension to iterate over multiple
lists at once.
...
As of version 0.3 tables and databases are first-class. To assign to
the variable C<db> a database called "factorials" using the C<mysql>
driver, connecting to the default port as the user C<web> write:
...
When a database is evaluated on the server a connection is
automatically opened if one is not already open.

To assign to the variable C<factorials> the table "factorials" with
fields C<i> and C<f> both of type C<Int> associated with the database
C<db> write:

  var factorials = table "factorials" with (i : Int, f : Int) from db;

The variable C<factorials> has type C<TableHandle((i:Int,
f:Int),(i:Int, f:Int))>.  The first argument to C<TableHandle> is the
type for reading from the table and the second is the type for
writing. In this instance they are the same.


To avoid writing C<asList> in a C<for> comprehension, you may use the
special C<E<lt>--> shorthand. 

For comprehensions support a further kind of generator C<z <-- t> for
reading from database tables. So with the above declarations you may
write
...
An entire table can be read using the C<asList> function:

  sig asList : (TableHandle((|a::Base),_,_)) -> [(|a::Base)]
  fun asList(t) {for (z <-- t) [z]}

As of version 0.5 table fields support two kinds of constraint:
C<readonly> and C<default>. A C<readonly> field must not be included
in an insert or an update. A C<default> field need not be included in
an insert.

To assign to the variable C<t> the table "people" with fields C<id> of
type C<Int>, C<name> of type C<String> and C<nationality> of type
C<String> where C<id> is readonly and C<nationality> has a default,
write:
...
The components of the C<TableHandle> type indicate the I<readable>
fields, the I<writeable> fields and the I<needed> fields. Only C<name>
and C<nationality> are writeable as C<id> is readonly. Only C<name> is
needed (in order to insert a row into the "people" table) as
C<nationality> has a default and C<id> is readonly.

=head2 Database abstraction

Links version 0.5 takes a radically different approach to compiling
Links code to SQL. Older versions of Links failed to compile database
code to efficient SQL as soon as the programmer abstracted over parts
of the query (other systems such as LINQ have similar problems).

A simple example is abstracting over a where clause. The natural way
to do this in Links is to represent the where clause using a predicate
function mapping table rows to booleans:

  fun (p) {
    for (x <-- t) where (p(x)) e
  }

The problem is that if C<p> is allowed to be arbitrary code then there
is no way that the SQL compiler can know that it can be compiled to
SQL, so the only option is to compile the for comprehension to an SQL
query that reads the whole of C<t> and then performs the filtering in
Links. The solution implemented in Links as of version 0.5 is to use an
effect system to restrict the type of C<p> to only admit functions
which are compileable to SQL.

Function arrows are annotated with effects. A straight arrow C<->>
indicates a I<tame> function, that is a function that can be compiled
to the database. A squiggly arrow C<~>> indicates a I<wild>
function, that is one that cannot be compiled to the database.

When you write a for comprehension with a table generator in it that
automatically forces any functions from the body and any C<where> or
C<orderby> clauses to be tame. If you try to call a wild function from
inside one of these clauses then you will get a type
error.

Furthermore, the type system also includes a simple subkinding system
which is used to enforce the constraint that a database comprehension
can only return a list of records of base type. (Type variables either
have kind C<Any>, which means they can be instantiated to any type, or
kind C<Base> which means they can only be instantiated to base
types. The kind C<Base> is a subkind of C<Any>.)  This ensures that a
database comprehension always compiles into exactly one SQL query.

=head2 Query blocks

A query block supports further abstraction over queries. 

  query {
    e
  }

The expression C<e> must be tame and must have the type list of record
of base type.
...
=head4 Special features: database compilation

Certain forms of regex can be compiled into an SQL test using the
C<LIKE> operator. For example, the expression

    for (x <-- t)
    where (x.name ~ /.*edinburgh.*/)
      [x.id]

should compile to an SQL query that selects all those C<id>s from the
table pointed to by C<t> having C<edinburgh> as a substring of their
C<name> column. The query would look like this:
...
prefixing a type variable with a question mark C<?>. Flexible type
...
the pretty-printer. The meta-variable C<X> stands for a rigid or
...
C<select(list, i)> returns the C<i>th element of the list, the first
...
Functions for arbitrary list manipulation. (Documentation TBD.)
...
Constructs a formlet containing a multiple-select scrollbox containing
