...
class m5_dbresult (conn : Mapi.t) (handle : Mapi.handle) = object
  ...
  method ntuples : int = Int64.to_int (Mapi.get_row_count handle) 
  ...
  method getvalue : int -> int -> string = fun row field ->
    match Mapi.seek_row handle (Int64.of_int row) Mapi.SEEK_SET with
      | Mapi.MOK ->
	  begin
	    match Mapi.fetch_row handle with
	      | Some nr_fields ->
		  begin
		    match Mapi.fetch_field handle field with
		      | Some s -> s
		      | None -> failwith ("MonetDB fetch_field failed")
		  end
	      | None -> failwith ("MonetDB fetch_row failed")
	  end
      | _ -> 
	  let err_str = from_option "no error string" (Mapi.result_error handle) in
	    failwith ("MonetDB seek_row failed: "^err_str)
