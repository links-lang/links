  ...
  Editor.prototype = {
    ...
    highlight: function(from, lines, cleanLines){
      ...
      function tokenPart(token){
        var tName = (token.style.indexOf("variable") > -1) ? 
                       "variable" : token.style;
        var part = self.doc.createElement("SPAN");
        part.className = "part " + token.style;
        part.setAttribute("name", tName);
      ...
      forEach(parsed, function(token){
        ...
        if (token.value == "\n"){
	      ...
	      // We're out of synch here, but don't stress
            throw StopIteration; //"Parser out of sync. Expected SPAN.";
          if (part.dirty)
            lineDirty = true;
          lineHasNodes = true;
      ...
      if (!this.parent.frame.name.match(/\*$/))
        this.parent.frame.name += "*";

      var voidVar = this.parent.frame.name;
