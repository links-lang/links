...
struct
  ...
  and shouter a p : t -> t list =
    function
      ...
      | e ->
        Debug.print ("Can't apply shouter to: " ^ Show.show show_t e);
        assert false
...
(* The problem is we re-traverse `List vs once per a,d pair. *)
  ...
  let stitch_query_old : Value.t package -> Value.t =
    fun p ->
      stitch_old (`Record [("1", `Int (Num.num_of_int top)); ("2", `Int (Num.num_of_int 1))]) p
...
(* Used in FastStitching also. *)
 ...
 let rec stitch_new : Value.t -> Value.t list IntPairMap.t package -> Value.t =
  ...
  let stitch_mapped_query : Value.t list IntPairMap.t package -> Value.t = 
      stitch_new (`Record [("1", `Int (Num.num_of_int top)); 
...
let table_field_types (_, _, _, (fields, _)) = StringMap.map snd fields
...
struct
  ...
  let rec lins_inner (z, z_fields) ys : t -> t =
    function
      ...
      | `Constant c      -> `Constant c
      | e ->
        Debug.print ("Can't apply lins_inner to: " ^ Show.show show_t e);
        assert false
      ...
      let liq = lins_inner_query (z, z_fields) ys in
        match e with
          ...
          *)
          ...
          | e -> 
            Debug.print ("Can't apply lins_inner_query to: " ^ Show.show show_t e);
            assert false
      ...
      | `Record fields ->
        ...
        `Record
          (StringMap.fold
             ...
             StringMap.empty)
      | e ->
        Debug.print ("Can't apply flatten_inner to: " ^ Show.show show_t e);
        assert false
...
(* keeing this version around since it seems slightly faster in some cases, should try to find out why*)
  let build_stitch_map_old (((vs:Value.dbvalue),rs),t) = 
    ...
    vs#fold_array (fun row (m) -> 
      ...
      | `Record [("1", (`Record [("1", `Int a); ("2", `Int d)])); 
		  ("2", w)] ->
		    Shred.insert (Num.int_of_num a,Num.int_of_num d) w m
...
(*    Debug.print ("clause: "^string_of_t v); *)
  ...
  and base : Value.database -> index -> t -> base = fun db index ->
    function
      ...
      | e ->
        Debug.print ("Not a base expression: " ^ Show.show show_t e);
        assert false
