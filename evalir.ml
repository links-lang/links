...
(*                            fs) *)
  ...
  and apply cont env : Value.t * Value.t list -> unit =
    function
...
(*             Debug.print("receive in web server mode--not implemented."); *)
  ...
  and tail_computation env cont : Ir.tail_computation -> unit = function
  ...
  and special env cont : Ir.special -> unit = function
    | `Wrong _                    -> raise Wrong
  ...
  let eval : Value.env -> program -> unit =
