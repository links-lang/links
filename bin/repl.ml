    ...
    ((fun (envs) args ->
        match args with
          | [filename] ->
              ...
              let envs, _ = Evaluation.evaluate ~printer:print_value parse_and_desugar envs filename in
                envs
        ...
        Errors.display ~default:(fun _ -> envs)
          (lazy
             (match parse () with
                ...
                | `Expression (e, t), _ ->
                    let valenv, _ = Evaluation.process_program ~printer:print_value envs (e, t) [] in
                      valenv, nenv, tyenv
