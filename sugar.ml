     ...
     let apply_annotation : Syntax.position -> untyped_expression -> annotation * untyped_expression -> untyped_expression =
       fun pos exp ((names, datatypes), body) ->
         ...
         let body = List.fold_right (fun datatype body ->
                                      Let ("_", PatternHasType (exp, datatype, pos), body, pos)) datatypes body
         in
           body
       ...
       fun pos vars bs def var ->
		 ...
		 (match exp with
		    | Record_empty _ when Settings.get_value unit_hack ->
                        ...
                        *)
			Let ("_", PatternHasType (Variable (var, pos), Types.unit_type, pos),
...
(struct
   ...
   type xml_type_definition =
       { regexp : Xml.Definition.regexp;
         needs : StringSet.t;
         mutable satisfied : bool; }

   let defined_xml_types = Hashtbl.create 16

   let rec satisfy_definition name list =
     let definition =
       try Hashtbl.find defined_xml_types name
       with Not_found ->
         List.iter (fun (_, definition) -> definition.satisfied <- false) list;
         failwith (Printf.sprintf "Need a definition for %s." name) in
     if definition.satisfied then list
     else
       let list = (name, definition) :: list in
       definition.satisfied <- true;
       StringSet.fold satisfy_definition definition.needs list

   let satisfy_needs needs =
     let adapt_definition (name, definition) =
       name, Xml.Definition.of_regexp definition.regexp in
     Xml.Definition.define
       (List.map adapt_definition (StringSet.fold satisfy_definition needs []))

   let rec desugar_xml_type datatype =
     match datatype with
       | Latin1Type ->
           StringSet.empty, Xml.Definition.latin1
       | UnionType (left, right) ->
           let left_set, left_regexp = desugar_xml_type left
           and right_set, right_regexp = desugar_xml_type right in
           let used_type_set = StringSet.union left_set right_set in
           let xml_type = Xml.Definition.union left_regexp right_regexp in
           used_type_set, xml_type
       | ConcatType (left, right) ->
           let left_set, left_regexp = desugar_xml_type left
           and right_set, right_regexp = desugar_xml_type right in
           let used_type_set = StringSet.union left_set right_set in
           let xml_type = Xml.Definition.concat left_regexp right_regexp in
           used_type_set, xml_type
       | QuestionType contents ->
           let used_type_set, contents_regexp = desugar_xml_type contents in
           used_type_set, Xml.Definition.question contents_regexp
       | StarType contents ->
           let used_type_set, contents_regexp = desugar_xml_type contents in
           used_type_set, Xml.Definition.star contents_regexp
       | PlusType contents ->
           let used_type_set, contents_regexp = desugar_xml_type contents in
           used_type_set, Xml.Definition.plus contents_regexp
       | TextType text ->
           StringSet.empty, Xml.Definition.of_string text
       | ElementType (name, attr_list, attr_open, contents) ->
           let needs, contents_type =
             match contents with
               | [ PrimitiveType (`Abstract t) ] ->
                   StringSet.singleton t, Xml.Definition.of_name t
               | _ ->
                   let append (needs, contents_regexp) contents =
                     let needs', contents = desugar_xml_type contents in
                     let contents_regexp =
                       Xml.Definition.concat contents_regexp contents in
                     let needs = StringSet.union needs needs' in
                     needs, contents_regexp in
                   let needs, contents_regexp =
                     List.fold_left append
                       (StringSet.empty, Xml.Definition.epsilon) contents in
                   needs, Xml.Definition.of_regexp contents_regexp in
           let regexp =
             Xml.Definition.element name attr_list attr_open contents_type in
           needs, regexp
       | _ -> failwith "XML type cannot be mixed with other types"

   let define_xml_type name datatype =
     if Hashtbl.mem defined_xml_types name then
       failwith "Already declared.";
     let needs, regexp = desugar_xml_type datatype in
     let definition =
       { regexp = regexp;
         needs = needs;
         satisfied = false; } in
     Hashtbl.add defined_xml_types name definition
     ...
     let rec desugar varmap = 
       ...
       function datatype ->
         match datatype with
           ...
           | PrimitiveType (`Abstract name) ->
               if Hashtbl.mem defined_xml_types name then
                 begin
                   satisfy_needs (StringSet.singleton name);
                   let xml_type = Xml.Type.of_definition name in
                   `Primitive (`XML xml_type)
                 end
               else `Primitive (`Abstract name)
           ...
           | ElementType _ ->
               let needs, regexp = desugar_xml_type datatype in
               satisfy_needs needs;
               let xml_type = Xml.Type.of_regexp regexp in
               `Primitive (`XML xml_type)
   ...
   let rec get_type_vars : phrase -> quantifier list =
     ...
     and get_pattern_type_vars (p, _) = (* fold *)
       match p with 
         ...
         | `Question _
         | `Star _
         | `Plus _ -> failwith "Mixed patterns"
     in
       union -<- get_type_vars
     ...
     fun pat pos value body ->
       let rec pl pat pos value body =
		 ...
		 Let(name, value,
           ...
           | `HasType (pat, t) ->
               pl pat pos (PatternHasType (value, t, pos)) body
           | `Element _
           | `Text _
           | `Concat _
           | `Question _
     ...
     let rec desugar' lookup_pos ((s, pos') : phrase) : untyped_expression =
       ...
       and patternize = simple_pattern_of_pattern varmap lookup_pos in
         match s with
           ...
           | InfixAppl (`Name "@", e1, e2)  -> Xml_concat (desugar e1, desugar e2, pos)
...
(*                                  ], pos')), pos') *)
  ...
  sig 
    val define_xml_type : string -> datatype -> unit
...
type sentence' = (untyped_expression list, directive_or_type_decl) either
