...
let rec is_guarded : TypeVarSet.t -> alias_environment -> int -> datatype -> bool =
  fun bound_vars alias_env var t ->
    let isg = is_guarded bound_vars alias_env var in
    let isgr = is_guarded_row bound_vars alias_env var in
      match t with
        | `Not_typed -> true
        | `Primitive _ -> true
        | `MetaTypeVar point ->
              ...
              match Unionfind.find point with
                ...
                | `Recursive (var', t) ->
                    (var=var' || TypeVarSet.mem var' bound_vars) ||
                      is_guarded (TypeVarSet.add var' bound_vars) alias_env var t
                | `Body t -> isg t
            end
        | `Function (f, m, t) ->
            isg f && isg m && isg t
        | `ForAll (qs, t) -> 
            is_guarded (bind_quantifiers qs bound_vars) alias_env var t
        | `Record row -> isgr row
...
and is_guarded_row : TypeVarSet.t -> alias_environment -> int -> row -> bool =
  fun bound_vars alias_env var (field_env, row_var) ->
    is_guarded_row_var bound_vars alias_env var row_var
and is_guarded_row_var : TypeVarSet.t -> alias_environment -> int -> row_var -> bool =
  fun bound_vars alias_env var row_var ->
    match Unionfind.find row_var with
      ...
      | `Recursive (var', row) ->
          (var=var' || TypeVarSet.mem var' bound_vars) ||
            is_guarded_row (TypeVarSet.add var' bound_vars) alias_env var row
      | `Body row ->
          is_guarded_row bound_vars alias_env var row
...
let rec is_negative : TypeVarSet.t -> alias_environment -> int -> datatype -> bool =
  fun bound_vars alias_env var t ->
    let isp = is_positive bound_vars alias_env var in
    let isn = is_negative bound_vars alias_env var in
    let isnr = is_negative_row bound_vars alias_env var in
      match t with
        | `Not_typed -> false
        | `Primitive _ -> false
        | `MetaTypeVar point ->
              ...
              match Unionfind.find point with
                ...
                | `Recursive (var', t) ->
                    not (TypeVarSet.mem var' bound_vars) &&
                      is_negative (TypeVarSet.add var' bound_vars) alias_env var t
                | `Body t -> isn t
            end
        | `Function (f, m, t) ->
            isp f || isp m || isn t
        | `ForAll (qs, t) -> is_negative (bind_quantifiers qs bound_vars) alias_env var t
        | `Record row -> isnr row
...
and is_negative_row : TypeVarSet.t -> alias_environment -> int -> row -> bool =
  fun bound_vars alias_env var (field_env, row_var) ->
    is_negative_field_env bound_vars alias_env var field_env || is_negative_row_var bound_vars alias_env var row_var
and is_negative_field_env : TypeVarSet.t -> alias_environment -> int -> field_spec_map -> bool =
  fun bound_vars alias_env var field_env ->
    FieldEnv.fold (fun _ spec result ->
                      match spec with
                        | `Absent -> result
                        | `Present t -> result || is_negative bound_vars alias_env var t
                   ) field_env false
and is_negative_row_var : TypeVarSet.t -> alias_environment -> int -> row_var -> bool =
  fun bound_vars alias_env var row_var ->
    match Unionfind.find row_var with
      ...
      | `Recursive (var', row) ->
          not (TypeVarSet.mem var' bound_vars) &&
            is_negative_row (TypeVarSet.add var' bound_vars) alias_env var row
      | `Body row ->
          is_negative_row bound_vars alias_env var row

and is_positive : TypeVarSet.t -> alias_environment -> int -> datatype -> bool =
  fun bound_vars alias_env var t ->
    let isp = is_positive bound_vars alias_env var in
    let isn = is_negative bound_vars alias_env var in
    let ispr = is_positive_row bound_vars alias_env var in
      match t with
        | `Not_typed -> false
        | `Primitive _ -> false
        | `MetaTypeVar point ->
            begin
              match Unionfind.find point with
                ...
                | `Recursive (var', t) ->
                    not (TypeVarSet.mem var' bound_vars) &&
                      is_positive (TypeVarSet.add var' bound_vars) alias_env var t
                | `Body t -> isp t
            end
        | `Function (f, m, t) ->
            isn f || isn m || isp t
        | `ForAll (qs, t) -> is_positive (bind_quantifiers qs bound_vars) alias_env var t
        | `Record row -> ispr row
...
and is_positive_row : TypeVarSet.t -> alias_environment -> int -> row -> bool =
  fun bound_vars alias_env var (field_env, row_var) ->
    is_positive_field_env bound_vars alias_env var field_env || is_positive_row_var bound_vars alias_env var row_var
and is_positive_field_env : TypeVarSet.t -> alias_environment -> int -> field_spec_map -> bool =
  fun bound_vars alias_env var field_env ->
    FieldEnv.fold (fun _ spec result ->
                      match spec with
                        | `Absent -> result
                        | `Present t -> result || is_positive bound_vars alias_env var t
                   ) field_env false
and is_positive_row_var : TypeVarSet.t -> alias_environment -> int -> row_var -> bool =
  fun bound_vars alias_env var row_var ->
    match Unionfind.find row_var with
      ...
      | `Recursive (var', row) ->
          not (TypeVarSet.mem var' bound_vars) &&
            is_positive_row (TypeVarSet.add var' bound_vars) alias_env var row
      | `Body row ->
          is_positive_row bound_vars alias_env var row
