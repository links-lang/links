...
let%expect_test "Typecheck example file examples/AIPL14/LectureExamples/calc.links" =
  ...
  [%expect {|
    exit: 2
    Fatal error: exception (Sys_error "tests/examples_default.config: No such file or directory") |}]
...
let%expect_test "Typecheck example file examples/handlers/coins.links" =
  ...
  [%expect {|
    exit: 2
    Fatal error: exception (Sys_error "tests/examples_effect_sugar.config: No such file or directory") |}]
