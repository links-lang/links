...
let%expect_test "Non-recursive block-scope functions:" =
  run_expr {|{ var f = fun(x) { f(x) }; () }|};
  [%expect {|
    exit: 1<string>:1: Type error: Unknown variable f.
    In expression: f. |}]
...
let%expect_test "No value recursion" =
  ...
  [%expect {|
    exit: 1<string>:1: Type error: Unknown variable g.
    In expression: g. |}]
...
let%expect_test "Reject multiple occurrences of a name in a pattern [1]" =
  run_expr {|fun (x,x) { x }|};
  [%expect {|
    exit: 1<string>:1: Type error: Duplicate names are not allowed in patterns.
