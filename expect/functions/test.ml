...
let%expect_test "Incorrect type annotations rejected" =
  ...
  [%expect {|
    exit: 1<string>:1: Type error: The inferred type of the expression
        `fun (x) { x + 1 }'
    is
        `(Int) -a-> Int'
...
let%expect_test "Closure conversion: Test generalization of free type variables during hoisting" =
  ...
  [%expect {| exit: 1***: Module Error: Could not find file ./tests/functions/nested-functions-polymorphic.links |}]
...
let%expect_test "Closure conversion: Test function with free type variables, but no free term variables" =
  ...
  [%expect {| exit: 1***: Module Error: Could not find file ./tests/functions/closure-conv-type-abstr-only.links |}]

let%expect_test "Quantifiers should not escape their scopes (#687)" =
  run_file {|./tests/functions/escaped-quantifier.links|};
  [%expect {| exit: 1***: Module Error: Could not find file ./tests/functions/escaped-quantifier.links |}]
