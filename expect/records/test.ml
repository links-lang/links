...
let%expect_test "With syntax (missing label)" =
  ...
  [%expect {|
    exit: 1<string>:1: Type error: A record can only be updated with compatible fields, but the expression
        `(x = 3)'
    has type
        `(x:Int)'
...
let%expect_test "Duplicate fields" =
  run_expr {|(x=3,x=3)|};
  [%expect {|
    exit: 1<string>:1: Type error: Duplicate labels (x) in record.
    In expression: (x=3,x=3). |}]

let%expect_test "Uninhabited recursive rows (questionable)" =
  run_expr {|(fun (x : (|(mu b . b))) {x})(())|};
  [%expect {| exit: 1***: Error: "Assert_failure core/unify.ml:1046:11" |}]

let%expect_test "Missing absent label (1)" =
  run_expr {|fun (r : (|a)) {(l=42|r)}|};
  [%expect {|
    exit: 1<string>:1: Type error: Only a record can be extended, and it must be extended with different fields, but the expression
        `<dummy>'
    has type
        `(|a)'
