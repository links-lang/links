...
let%expect_test "Alien declaration below toplevel" =
  run_expr_here {|if (true) { alien javascript "foo.js" foo : () ~> (); foo() } else { () }|};
  [%expect {|
...
let%expect_test "Alien functions may be evaluated in the interpreter" =
  run_expr_here {|alien javascript "fun.js" f : () ~> (); id(f)|};
  [%expect {|
    alien : () ~> ()
...
let%expect_test "Alien values may be evaluated in the interpreter" =
  run_expr_here {|alien javascript "val.js" x : (); id(x)|};
  [%expect {|
    alien : ()
...
let%expect_test "Alien functions may not be applied in the interpreter" =
  run_expr_here {|alien javascript "fun.js" f : () ~> (); f()|};
  [%expect {| exit: 1***: Error: Links_core.Evalir.Exceptions.EvaluationError("Cannot make alien call on the server.") |}]

let%expect_test "Alien binders cannot contain primes" =
  run_expr_here {|alien javascript "" f' : () -> ();|};
  [%expect {|
    exit: 1<string>:1: Syntax error: Foreign binders cannot contain single quotes `'`.
