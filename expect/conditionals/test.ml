...
let%expect_test "Typing of the test" =
  ...
  [%expect {|
    exit: 1<string>:1: Type error: The condition of an `if (...) ... else ...' expression must have type `Bool', but the expression
        `3'
    has type
        `Int'
...
let%expect_test "Typing of both branches" =
  ...
  [%expect {|
    exit: 1<string>:1: Type error: Both branches of an `if (...) ... else ...' expression should have the same type, but the expression
        `"yes"'
    has type
        `String'
