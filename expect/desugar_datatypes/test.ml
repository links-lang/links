...
let%expect_test "Type variables are correctly scoped [1]" =
  ...
  [%expect {|
    exit: 1<string>:1: Type error: Mismatch in kind for type variable `a'.
...
let%expect_test "Type variables are correctly scoped [2]" =
  ...
  [%expect {| exit: 1***: Module Error: Could not find file ./tests/desugar_datatypes/shadowKinds.links |}]
...
let%expect_test "Quantifiers within nested definitions are allowed [1]" =
  ...
  [%expect {| exit: 1***: Module Error: Could not find file ./tests/desugar_datatypes/nestedQuantifiers1.links |}]
...
let%expect_test "Quantifiers within nested definitions are allowed [2]" =
  ...
  [%expect {| exit: 1***: Module Error: Could not find file ./tests/desugar_datatypes/nestedQuantifiers2.links |}]
...
let%expect_test "Qualified type variables can infer their kind if enabled [typename]" =
  ...
  [%expect {|
    exit: 2Fatal error: exception (Sys_error
...
let%expect_test "Free type variables are detected" =
  ...
  [%expect {|
    exit: 1<string>:1: Type error: Unbound type variable `a' in position where
...
let%expect_test "Implicit type variables are detected" =
  run_expr {|typename A = () -> ();|};
  [%expect {|
    exit: 1<string>:1: Type error: Unbound anonymous type variable in position where
            no free type variables are allowed
    In expression: () -> (). |}]
...
let%expect_test "Free type variables are detected (in nested definitions)" =
  ...
  [%expect {| exit: 1***: Module Error: Could not find file ./tests/desugar_datatypes/nestedTypename.links |}]

let%expect_test "Type aliases cannot repeat variables" =
  run_expr {|typename T(a::Type, a::Type) = a;|};
  [%expect {|
    exit: 1:0: Type error: Multiple definitions of type variable `a'.
    In expression: <dummy>. |}]

let%expect_test "foralls cannot repeat variables" =
  run_expr {|() : forall a::Type, a::Type. a|};
  [%expect {|
    exit: 1<string>:1: Type error: Multiple definitions of type variable `a'.
...
let%expect_test "Sugar for shared effect variables in functions" =
  ...
  [%expect {| exit: 2Fatal error: exception (Sys_error "tests/effect_sugar.config: No such file or directory") |}]
