...
let%expect_test "Top level operation invocation" =
  ...
  [%expect {|
    exit: 1<string>:1: Type error: Invocation of the operation
        `Foo'
...
let%expect_test "Return invocation (1)" =
  ...
  [%expect {|
    exit: 1<string>:1: Type error: The implicit effect Return is not invocable
...
let%expect_test "Exception handling (3)" =
  ...
  [%expect {|
    exit: 1<string>:1: Type error: The effect type of an input to a handle should match the type of its computation patterns, but the expression
...
let%expect_test "Pattern-matching on continuation parameter (3)" =
  ...
  [%expect {|
    exit: 1<string>:1: Type error: Improper pattern matching on resumption
    In expression: 2. |}]
...
let%expect_test "Pattern-matching on handler parameter (4)" =
  ...
  [%expect {|
    exit: 1<string>:1: Type error: The parameter pattern must match the expression in a handle parameter binding, but the pattern
        `Foo(s)'
    has type
        `[|Foo:a::Any|]'
