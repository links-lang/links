...
let%expect_test "HasType pattern [2]" =
  ...
  [%expect {|
    exit: 1<string>:1: Type error: All the cases of a switch should have compatible patterns, but the pattern
        `1:Int'
    has type
        `Int'
...
let%expect_test "Redundant pattern [2]" =
  ...
  [%expect {|
    exit: 1<string>:1: Type error: All the cases of a switch should have the same type, but the expression
        `A(1)'
    has type
        `[|A:Int|a::Any|]'
