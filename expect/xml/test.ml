...
let%expect_test "Rejection of incorrectly typed attributes" =
  ...
  [%expect {|
    exit: 1<string>:1: Type error: XML attributes must have type `String', but the expression
        `{x}'
    has type
        `Int'
...
let%expect_test "Rejection of incorrectly typed l:attributes" =
  ...
  [%expect {|
    exit: 1:0: Type error: The function
        `<dummy>'
    has type
        `(() -a-> Page) ~b~> String'
...
let%expect_test "Reject nonsense l:name attributes" =
  ...
  [%expect {|
    exit: 1<string>:1: Error compiling attributes: Illegal l: attribute in XML node
...
let%expect_test "Ill-formed namespace" =
  ...
  [%expect {| exit: 1***: Runtime error: Illegal character in namespace |}]

let%expect_test "Ill-formed tag-name [1]" =
  run_expr {|variantToXmlItem(Node("tag:name", [ ]))|};
  [%expect {| exit: 1***: Runtime error: Illegal character in tagname |}]
...
let%expect_test "Setting illformed Attributes [1]" =
  ...
  [%expect {| exit: 1***: Runtime error: Attribute names cannot contain colons. |}]

let%expect_test "Setting illformed Attributes [2]" =
  run_expr {|setAttribute(makeXml("foo", [ ], <#/>), "ill:formed", "baz")|};
  [%expect {| exit: 1***: Runtime error: Attribute names cannot contain colons. Use setAttributeNS instead. |}]
...
let%expect_test "Ill-bracketed tag (opening tag hidden in a comment)" =
  run_expr {|<!-- <b> --></b>|};
  [%expect {|
    exit: 1<string>:1: Type error: Unknown variable </.
    In expression: </b. |}]

let%expect_test "Newlines in comment" =
  run_file {|./tests/xml_comment_newlines.links|};
  [%expect {| exit: 1***: Module Error: Could not find file ./tests/xml_comment_newlines.links |}]
