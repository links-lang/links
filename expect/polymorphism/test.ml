...
let%expect_test "Value restriction [1]" =
  ...
  [%expect {|
    exit: 1<string>:1: Type error: Because of the value restriction there can be no
...
let%expect_test "Overly general signatures" =
  ...
  [%expect {|
    exit: 1<string>:1: Type error: The non-recursive function definition has return type
        `a'
...
let%expect_test "Missing quantifier with mixed kinds (1)" =
  ...
  [%expect {|
    exit: 1<string>:1: Type error: Unbound type variable `e' in position where
