...
module TMap : sig
    ...
    | None ->
        let st =
          ...
          | TTuple elems ->
              ...
              Some (SubT (Final, [||], DefStructT (StructT (Array.map (fun _ -> FieldT (Cons, ValStorageT (RefT (Null, EqHT)))) (Array.of_list elems)))))
  ...
  let sched_effects (env : t) : int32 list = let open Wasm.Type in [
    (* spawn *) recid_of_sub_type env (SubT (Final, [||], DefFuncT (
      FuncT ([|RefT (NoNull, VarHT (recid_of_type env (TClosed (TLnil, TVar))))|], [|RefT (NoNull, VarHT pid_tid)|]))));
    (* self *) recid_of_sub_type env (SubT (Final, [||], DefFuncT (FuncT ([||], [|RefT (NoNull, VarHT pid_tid)|]))));
    (* yield *) recid_of_sub_type env (SubT (Final, [||], DefFuncT (FuncT ([||], [||]))));
    (* wait *) recid_of_sub_type env (SubT (Final, [||], DefFuncT (FuncT ([|RefT (NoNull, VarHT pid_active_tid)|], [|RefT (Null, EqHT)|]))));
  ]
end
type tmap = TMap.t
...
module NewMetadata : sig
  ...
  val wasm_locals_list : tmap -> t -> Wasm.Type.val_type list
  ...
  let wasm_of_exports ({ exps; gbls; _ } : g) : Wasm.global list =
    let fglobals =
      Int32Map.fold
        (fun fid (b, ftid) acc ->
          if b then (Wasm.Type.(GlobalT (Cons, RefT (NoNull, VarHT ftid))), Wasm.Instruction.[|RefFunc fid|], None) :: acc
          else acc)
        exps []
        |> List.rev in
  ...
  let wasm_locals_list (_ : tmap) (_, { contents = (l, _) }, _ : t) : Wasm.Type.val_type list =
    List.rev l
...
let generate_yield (glob : NewMetadata.g) (procinfo : procinfo) : instr_conv = match procinfo with
  ...
  | Some (procinfo, _, yield_fun) -> let open Wasm in let open Value in let open Type in let open Instruction in
      let do_check_yield = fun acc ->
        If (ValBlockType None,
          convert_nlist (generate_real_yield glob empty_nlist)
        , [|
          GlobalGet procinfo;
          Const (I64 I64.one);
          Binop (I64 IntOp.Sub);
...
and convert_expr : type a b. _ -> _ -> _ -> a expr -> (a, b) box -> _ -> _ -> instr_conv =
  ...
  | ECallClosed (ESpecialize (EClose (f, bcl, cls), _, bargs, bret), args, _) ->
      ...
      | Some (refid, jmplv) when can_early_ret && Int32.equal fid (refid :> int32) ->
          ...
          fun acc -> Br jmplv ^+ generate_yield (NewMetadata.g_of_t new_meta) procinfo (set_args (gen_new_struct (args acc)))
      | _ ->
  ...
  | ECallClosed (ESpecialize (ERawClose (f, cl), _, bargs, bret), args, _) ->
      ...
      | Some (refid, jmplv) when can_early_ret && Int32.equal fid (refid :> int32) ->
          ...
          let set_args acc =
            ...
            let nargs = inner targs 0 in
            (nargs, List.init nargs (fun i -> LocalSet (Int32.of_int i))) @+ acc in
          fun acc -> Br jmplv ^+ generate_yield (NewMetadata.g_of_t new_meta) procinfo (set_args (get_cl (args acc)))
      | _ ->
          let unbox = do_unbox tm new_meta bret (fun acc ->
  ...
  | EDo (eid, tret, args) ->
      ...
      let ret = really_unbox tm tret (fun acc -> Suspend eid ^+ generate_yield (NewMetadata.g_of_t new_meta) procinfo (args acc)) in
    ...
    Wasm.{
      ...
      fn_code = let open Value in let open Type in Instruction.[|
        ...
        Block (ValBlockType (Some (RefT (NoNull, VarHT TMap.list_tid))), [|
          ...
          Loop (ValBlockType (Some (RefT (NoNull, VarHT TMap.list_tid))), [|
...
let convert_fun (tm : tmap) (glob : NewMetadata.g) (procinfo : procinfo) (f : ('a, 'b) func') : int32 option * Wasm.fundef =
  ...
  convert_fun_aux
    tm glob procinfo fun_typ nparams (List.map (fun (Type t) -> TMap.val_of_type tm t) f.fun_locals) f.fun_block
...
let convert_fst (tm : tmap) (glob : NewMetadata.g) (procinfo : procinfo) (f : 'b fstart) : int32 option * Wasm.fundef =
  ...
  convert_fun_aux
    tm glob procinfo fun_typ 1l (List.map (fun (Type t) -> TMap.val_of_type tm t) f.fst_locals) f.fst_block
  ...
  let procinfo =
    if m.mod_has_processes then begin
      ...
      let angels_vid = NewMetadata.add_raw_global glob (
        Type.(GlobalT (Var, RefT (Null, VarHT TMap.pid_list_tid))),
        Instruction.[|RefNull Type.(VarHT TMap.pid_list_tid)|],
        None) in
          ...
          let rec prepare : type a. a typ -> _ = fun (t : a typ) (new_meta : new_meta) ->
            ...
            | TList t ->
                let mainblocktid = TMap.recid_of_sub_type tm Type.(SubT (Final, [||],
                      DefFuncT (FuncT ([|RefT (Null, VarHT TMap.list_tid)|], [||])))) in
                let locid = NewMetadata.add_raw_local new_meta Type.(RefT (NoNull, VarHT TMap.list_tid)) in
                let unbox =
                  (@<) (really_unbox tm t (fun acc -> StructGet (TMap.list_tid, 0l, None) ^+ LocalGet locid ^+ acc) empty_nlist) in
                ...
                Either.Left (nlist_of_list [
                  ...
                  Block (Type.(VarBlockType mainblocktid), [|
                    BrOnNull 0l;
                    LocalSet locid;
                    Loop (Type.(ValBlockType None), convert_straight_nlist (
                      unbox (
                      print_elem (nlist_of_list [
                      LocalGet locid;
                      StructGet (TMap.list_tid, 1l, None);
                      BrOnNull 1l;
                      LocalSet locid;
            ...
            | TClosed _ ->
                let add_string code s =
                  String.fold_right (fun c acc -> Const (I32 (I32.of_int_s (Char.code c))) ^+ Call putc ^+ acc)
                    s code in
                Either.Left (Drop ^+ add_string empty_nlist "fun")
            | _ -> failwith "TODO: Irtowasm.compile.prepare for this type"
          in
          let code = prepare m.mod_main new_meta in
          let code = match code with
              | Either.Left code -> code @< nlist_of_list [GlobalGet m.mod_nglobals; GlobalSet m.mod_nglobals]
              | Either.Right fid -> nlist_of_list [Call fid; GlobalGet m.mod_nglobals; GlobalSet m.mod_nglobals] in
          let code =
            let add_string code s =
              String.fold_left (fun acc c -> Call putc ^+ Const (I32 (I32.of_int_s (Char.code c))) ^+ acc)
                code s in
            add_string (add_string code " : ") main_typ_name in
          glob, Call putc ^+ Const (I32 (I32.of_int_s (Char.code '\n'))) ^+ code, NewMetadata.wasm_locals_list tm new_meta in
    let locals =
  ...
  let mainid =
    ...
    | Some (procinfo, angel_list, _) ->
      ...
      let spawn_bt = TMap.recid_of_sub_type tm (SubT (Final, [||], DefFuncT (
        FuncT ([||], [|RefT (NoNull, VarHT spawn_cbid); RefT (NoNull, VarHT spawn_contid)|])))) in
      ...
      initref := Some {
        ...
        fn_locals = [|
          ...
          RefT (NoNull, VarHT TMap.process_waiting_tid); (* 7: $wait continuation *)
          RefT (NoNull, VarHT TMap.pid_active_tid);      (* 8: $spawn/$wait PID / main/returned process PID *)
        |];
            ...
            Loop (ValBlockType None, [|
              Block (ValBlockType None, [|                                                       (* Load next process *)
                Block (VarBlockType wait_bt, [|                                                  (* $wait *)
                  Block (ValBlockType (Some (RefT (NoNull, VarHT TMap.process_active_tid))), [|  (* $yield *)
                    Block (VarBlockType spawn_bt, [|                                             (* $spawn *)
                      Block (ValBlockType (Some (RefT (Null, EqHT))), [|                         (* (returned) *)
                        Block (ValBlockType (Some (RefT (NoNull, VarHT self_ctid))), [|          (* $self *)
              ...
              *)
              Loop (ValBlockType None, [|
                ...
                StructGet (TMap.process_list_tid, 0l, None);
                StructGet (TMap.pid_tid, 1l, None);
                RefTest (NoNull, VarHT TMap.pid_active_tid);
                Testop (I32 IntOp.Eqz);
