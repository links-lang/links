...
let rec convert_block : 'a. _ -> 'a block -> _ -> _ -> _ -> _ -> tinstr_conv =
  fun (type a) (tm : tmap) ((ass, e) : a block) (is_last : bool) (ctid : int32) (cid : int32) (can_jmp : (mfunid * int32) option) : tinstr_conv ->
  let open Wasm.Instruction in
  let rec inner tm (ass : assign list) : tinstr_conv = match ass with
    | [] -> convert_expr tm e is_last ctid cid can_jmp
    | Assign (l, v, e) :: tl ->
        let _, v = (v : _ varid :> (_ typ * int32)) in
        let tm, t, f = inner tm tl in
        let tm, _, e = convert_expr tm e false ctid cid None in
  ...
  in inner tm ass
and convert_expr : 'a. _ -> 'a expr -> _ -> _ -> _ -> _ -> tinstr_conv =
    fun (type a) (tm : tmap) (e : a expr) (is_last : bool) (ctid : int32) (cid : int32) (can_jmp : (mfunid * int32) option) : tinstr_conv ->
  ...
  | EUnop (op, e) ->
      let tm, _, arg = convert_expr tm e false ctid cid None in
      convert_unop tm op arg
  | EBinop (op, e1, e2) ->
      let tm, argt1, arg1 = convert_expr tm e1 false ctid cid None in
      let tm, argt2, arg2 = convert_expr tm e2 false ctid cid None in
  ...
  | ECallRawHandler (fid, _, contarg, targs, args, hdlarg, tret) ->
      ...
      let tm, _, hdlarg = convert_expr tm hdlarg false ctid cid None in
      let tm, _, contarg = convert_expr tm contarg false ctid cid None in
  ...
  | ECond (rt, e, t, f) ->
      let tm, _, ti = convert_block tm t is_last ctid cid can_jmp in
      let tm, _, fi = convert_block tm f is_last ctid cid can_jmp in
      let tm, _, ei = convert_expr tm e false ctid cid None in
  ...
  | ECont (loc, contid, contargs, hdlfid, hdlclloc, hdlclid) -> begin
      ...
      | Some (refid, jmplv) when Int32.equal hdlfid (refid :> int32) ->
          ...
          tm, Type tret, fun acc ->
            Br jmplv ::
            convert_get_var loc contid ctid cid (
            args acc)
      | _ ->
...
and convert_exprs : 'a. _ -> 'a expr_list -> _ -> _ -> tmap * instr_conv =
  ...
  | ELcons (ehd, etl) ->
      let tm, _, f = convert_expr tm ehd false ctid cid None in
...
let convert_anyblock (tm : tmap) (Block (_, b) : anyblock) (is_last : bool) (ctid : int32) (cid : int32) : tmap * Wasm.Instruction.t list =
  let tm, _, b = convert_block tm b is_last ctid cid None in tm, b []
let convert_finisher (type a b) (tm : tmap) (f : (a, b) finisher) (is_last : bool) (ctid : int32) (cid : int32) can_jmp : tmap * Wasm.Instruction.t list =
  match f with
  | FId _ -> tm, []
  | FMap (v, _, b) ->
      let _, v = (v : _ varid :> _ * int32) in
      let tm, _, b = convert_block tm b is_last ctid cid can_jmp in
...
let convert_hdl (tm : tmap) (type a b c) (f : (a, b, c) fhandler) : tmap * Wasm.fundef =
  ...
  in let tm, code =
    ...
    let tm, code = convert_finisher tm f.fh_finisher true clostyp cid None in
    ...
    let rec do_cases tm nblocks (cases : (a, b) handler list) code = match cases with
      ...
      | Handler (type ea er) (eid, varc, vars, blk : (ea, er) effectid * _ * _ * _) :: tl ->
          ...
          let tm, codehdl =
            let tm, _, b = convert_block tm blk true clostyp cid (Some (f.fh_id, nblocks)) in
