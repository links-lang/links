    ...
    object (o : 'self) inherit IrTraversals.Transform.visitor(tenv) as super
      ...
      method! binding =
        ...
        | (Let (_, (quantifiers, _))) as b->
          ...
          let o = List.fold_left (fun o q -> o#quantifier_remove q) o quantifiers in
          (b, o)
        | (Fun (f, (tyvars, xs, body), None, unsafe, location)) as b
          ...
          Fun (f, (tyvars, xs, body), None, unsafe, location), o
        ...
        | (Rec defs) as b when Scope.isLocal (Ir.binding_scope b) ->
          ...
          let defs, o =
            List.fold_left
              (fun (defs, (o : 'self))
                   (f, (tyvars, xs, body), none, unsafe, location) ->
                 assert (none = None);
                 let o = List.fold_left (fun o q -> o#quantifier q) o tyvars in
                 let xs, o =
                   List.fold_right
                 ...
                 (f, (tyvars, xs, body), None, unsafe, location)::defs, o)
              ([], o)
              defs in
...
struct
  ...
  let fun_def (f, lam, z, unsafe, location) =
    (binder f, lam, z, unsafe, location)
    ...
    object (o : 'self) inherit IrTraversals.Transform.visitor(tenv) as super
      ...
      method! bindings =
        ...
        | b :: bs when Scope.isGlobal (Ir.binding_scope b) ->
          ...
          let bs, o = o#bindings bs in
          bs' @ (b :: bs), o
        | Fun ((f, _) as fb, (tyvars, xs, body), None, unsafe, location) :: bs ->
          ...
          let fundef = o#generalize_function_body_for_hoisting
                         (fb, (tyvars, xs, body), zb, unsafe, location) in
        ...
        | Rec defs :: bs ->
            ...
            let fbs, defs, o =
              List.fold_right
                (fun (f, (tyvars, xs, body), zb, unsafe, location) (fs, defs,  o) ->
                   ...
                   let def = (f, (tyvars, xs, body), zb, unsafe, location) in
                     (f::fs, def::defs, o))
                defs
                ([], [], o) in
            ...
            let defs, o =
              List.fold_left
                (fun (defs, (o : 'self))
                     ((f, _) as fb, (tyvars, xs, body), none, unsafe, location) ->
                   assert (none = None);
                   assert (Scope.isLocal (Var.scope_of_binder fb));
                   let fb = Globalise.binder fb in
                   let xs, o =
      ...
      method generalize_function_body_for_hoisting : Ir.fun_def ->  Ir.fun_def = fun fundef ->
        let (f, (tyvars, xs, body), z, unsafe, location) = fundef in
        ...
        if free_type_vars = [] then
          ...
          begin
            ...
            (f, (tyvars, xs, body), z, unsafe, location)
          end
