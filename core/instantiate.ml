 ...
 *)
let instantiate : FrontendTypeEnv.qual_var_environment -> string -> type_arg list * datatype =
  fun env var ->
    let (_, t) =
      try
...
let rigid : FrontendTypeEnv.qual_var_environment -> string -> type_arg list * datatype =
  fun env var ->
    let (_, t) =
      try
        Env.String.lookup env var
