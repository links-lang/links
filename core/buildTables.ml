...
struct
  ...
  let make_eval_def : Ir.fun_def -> Ir.eval_fun_def =
    fun ((_f, info), (_tyvars, xs, body), z, unsafe, location) ->
      info, (List.map Var.var_of_binder xs, body),
      opt_map Var.var_of_binder z, unsafe, location
    ...
    method close_cont fvs : Ir.binding list -> 'self_type =
      function
        ...
        | Rec defs::bs ->
            let fvs, bound_vars =
              List.fold_right
                (fun (f, (_tyvars, xs, _body), z, _, _) (fvs, bound_vars) ->
            ...
            let fvs' =
              List.fold_left
                (fun fvs' (_f, (_tyvars, _xs, body), _zs, _, _location) ->
