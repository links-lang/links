...
let string_of_row ?(policy=default_policy) ?(refresh_tyvar_names=true) row =
  if Settings.get_value Basicsettings.print_types_pretty then
    let policy = policy () in
    build_tyvar_names ~refresh_tyvar_names Vars.free_bound_row_type_vars [row];
    let context = context_with_shared_effect policy (fun o -> o#row row) in
    row "," context (policy, tyvar_name_map) row
  else
    raw_show_row (DecycleTypes.row row)

let string_of_presence ?(policy=default_policy) ?(refresh_tyvar_names=true)
                       (f : field_spec) =
  build_tyvar_names ~refresh_tyvar_names Vars.free_bound_field_spec_type_vars [f];
  presence empty_context (policy (), tyvar_name_map) f

let string_of_type_arg ?(policy=default_policy) ?(refresh_tyvar_names=true)
                       (arg : type_arg) =
  let policy = policy () in
  build_tyvar_names ~refresh_tyvar_names Vars.free_bound_type_arg_type_vars [arg];
  let context = context_with_shared_effect policy (fun o -> o#type_arg arg) in
  type_arg context (policy, tyvar_name_map) arg

let string_of_row_var ?(policy=default_policy) ?(refresh_tyvar_names=true) row_var =
  build_tyvar_names ~refresh_tyvar_names Vars.free_bound_row_var_vars [row_var];
  match row_var name_of_type "," empty_context (policy (), tyvar_name_map) row_var
  with | None -> ""
       | Some s -> s

let string_of_tycon_spec ?(policy=default_policy) ?(refresh_tyvar_names=true) (tycon : tycon_spec) =
  build_tyvar_names ~refresh_tyvar_names Vars.free_bound_tycon_type_vars [tycon];
...
let string_of_quantifier ?(policy=default_policy) ?(refresh_tyvar_names=true) (quant : quantifier) =
  build_tyvar_names ~refresh_tyvar_names Vars.free_bound_quantifier_vars [quant];
