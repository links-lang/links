...
module PrettyWithPolicy : sig
  ...
  val build_tyvar_names :
       refresh_tyvar_names:bool
    -> ('a -> FreeTypeVars.vars_list)
    -> 'a list
    -> unit

  val add_tyvar_names : ('a -> FreeTypeVars.vars_list) -> 'a list -> unit

  val string_of_datatype :
    ?policy:(unit -> policy) -> ?refresh_tyvar_names:bool -> datatype -> string

  val string_of_row :
    ?policy:(unit -> policy) -> ?refresh_tyvar_names:bool -> row -> string

  val string_of_field_spec :
       ?policy:(unit -> policy)
    -> ?refresh_tyvar_names:bool
    -> field_spec
    -> string

  val string_of_type_arg :
    ?policy:(unit -> policy) -> ?refresh_tyvar_names:bool -> type_arg -> string

  val string_of_row_var :
    ?policy:(unit -> policy) -> ?refresh_tyvar_names:bool -> row_var -> string

  val string_of_tycon_spec :
       ?policy:(unit -> policy)
    -> ?refresh_tyvar_names:bool
    -> tycon_spec
    -> string
