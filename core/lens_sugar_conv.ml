...
module Error = struct
  type t = Internal_error of string
    [@@deriving show]
...
let is_static _typ p =
  let rec no_ext_deps v p =
    ...
    | S.InfixAppl (_, left, right) ->
      no_ext_deps v left && no_ext_deps v right
    | S.Var v' ->
      S.Binder.to_name v = v'
    | S.Projection (body, _) ->
      no_ext_deps v body
    | S.Constant _ -> true
    | _ -> false
    ...
    | S.Projection (var, field )->
      (match var |> WithPos.node with
       | S.Var v' ->
         if v = v'
         then Result.return ()
         else Format.asprintf "Unexpected external variable: %s" v' |> Error.internal_error_res
       | _ -> Format.asprintf "Unexpected expression to project on: %a" S.pp_phrase var |> Error.internal_error_res)
      >>| fun () ->
      LPS.Var field
    | _ -> Format.asprintf "Unsupported sugar phrase in lens sugar phrase of body: %a" S.pp_phrase p |> Error.internal_error_res )
  >>| fun v -> (pos, v)
...
let lens_sugar_phrase_of_sugar p =
  ...
  | S.FunLit (_, _, ([[var]], body), _) ->
    ...
    (match var with
     ...
     | _ -> Format.asprintf "Unsupported binder: %a" S.pp_phrase p |> Error.internal_error_res)
