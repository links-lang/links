  ...
  let t =
    if StringMap.mem name field_env then
      match (StringMap.find name field_env) with
        | `Present t -> t
        | `Absent ->
            error ("Attempt to split row "^ TP.string_of_row row ^" on absent field " ^ name)
        | `Var _ ->
            error ("Attempt to split row "^TP.string_of_row row ^" on var field " ^ name)
    else
      error ("Attempt to split row "^TP.string_of_row row ^" on absent field " ^ name)
  in
...
let rec variant_at ?(overstep_quantifiers=true) name t = match (concrete_type t, overstep_quantifiers) with
  ...
  | (t, _) ->
      error ("Attempt to deconstruct non-variant type "^TP.string_of_datatype t)
...
let rec split_variant_type name t = match concrete_type t with
  ...
  | t ->
      error ("Attempt to split non-variant type "^TP.string_of_datatype t)
...
let rec project_type ?(overstep_quantifiers=true) name t = match (concrete_type t, overstep_quantifiers) with
  ...
  | (`Record row, _) ->
      let t, _ = split_row name row in
        t
  | (t, _) ->
      error ("Attempt to project non-record type "^TP.string_of_datatype t)
...
let rec select_type name t = match concrete_type t with
  ...
  | `Select row ->
    let t, _ = split_row name row in t
  | t ->
    error ("Attempt to select from non-selection type "^ TP.string_of_datatype (concrete_type t))
...
let rec split_choice_type name t = match concrete_type t with
  ...
  | t ->
      error ("Attempt to split non-choice type "^ TP.string_of_datatype t)
...
let rec choice_at name t = match concrete_type t with
  ...
  | `Choice row ->
      let t, _ = split_row name row in t
  | t ->
      error ("Attempt to deconstruct non-choice type "^ TP.string_of_datatype t)
...
let rec erase_type ?(overstep_quantifiers=true) names t =
      ...
      let field_env =
        StringSet.fold
          (fun name field_env ->
            ...
            | Some `Absent ->
              error ("Attempt to remove absent field "^name^" from row "^TP.string_of_row row)
            | Some (`Var _) ->
              error ("Attempt to remove var field "^name^" from row "^TP.string_of_row row)
            | None ->
              error ("Attempt to remove absent field "^name^" from row "^TP.string_of_row row))
          names
          field_env
  ...
  | (t, _) -> error ("Attempt to erase field from non-record type "^ TP.string_of_datatype t)
...
let rec return_type ?(overstep_quantifiers=true) t = match (concrete_type t, overstep_quantifiers)  with
  ...
  | (`Lolli (_, _, t), _) -> t
  | (t, _) ->
      error ("Attempt to take return type of non-function: " ^ TP.string_of_datatype t)
...
(* *\) (\*error ("arg_types: " ^ (string_of_datatype t') ^ ", ret: " ^ TP.string_of_datatype t'')*\) *)
  | (t, _) ->
     error ("Attempt to take arg types of non-function: " ^ TP.string_of_datatype t)
...
let rec effect_row ?(overstep_quantifiers=true) t = match (concrete_type t, overstep_quantifiers)  with
  ...
  | (t, _) ->
      error ("Attempt to take effects of non-function: " ^ TP.string_of_datatype t)
...
let rec element_type ?(overstep_quantifiers=true) t = match (concrete_type t, overstep_quantifiers) with
  ...
  | (t, _) ->
      error ("Attempt to take element type of non-list: " ^ TP.string_of_datatype t)

let rec table_read_type t = match concrete_type t with
  | `ForAll (_, t) -> table_read_type t
  | `Table (r, _, _) -> r
  | t ->
      error ("Attempt to take read type of non-table: " ^ TP.string_of_datatype t)

let rec table_write_type t = match concrete_type t with
  | `ForAll (_, t) -> table_write_type t
  | `Table (_, w, _) -> w
  | t ->
      error ("Attempt to take write type of non-table: " ^ TP.string_of_datatype t)

let rec table_needed_type t = match concrete_type t with
  | `ForAll (_, t) -> table_needed_type t
  | `Table (_, _, n) -> n
  | t ->
      error ("Attempt to take needed type of non-table: " ^ TP.string_of_datatype t)
