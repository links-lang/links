...
type qual_tycon_environment =
  (QualifiedName.t option * Types.tycon_spec) Env.String.t
[@@deriving show]
...
let normalise_typing_environment env =
  ...
  { env with
    var_env=
      Env.String.map
        (fun (open_path, t) -> (open_path, Types.normalise_datatype t))
        env.var_env
  ; module_env=
      Env.String.map
        (fun (open_path, mt) -> (open_path, normalise_module_t mt))
        env.module_env
...
let extend_typing_environment ?effects
  ...
  ; effect_row= (match effects with None -> er | Some se -> se) }
...
let resolve_qualified_name (env : t) (qname : QualifiedName.t)
    (env_extractor : string -> t -> 'a option)
    (module_extractor : string -> Types.module_t -> 'b option) :
    ('a, 'b) resolve_result =
  let rec traverse_submodules orig_module count cur_qname
    ...
    | QualifiedName.Dot (mod_name, remainder) ->
        let next_module =
          StringMap.find_opt mod_name cur_module_t.Types.modules
        in
        begin match next_module with
        | None ->
            let module_path_until_failure =
              QualifiedName.prefix (count + 1) qname
            in
            raise (ModuleNotFound module_path_until_failure)
        | Some module_t ->
            traverse_submodules orig_module (count + 1) remainder module_t
        end
  in
  match qname with
  | QualifiedName.Ident name ->
  ...
  | QualifiedName.Dot (first_module, remainder) ->
    ...
    | None -> raise (ModuleNotFound (QualifiedName.of_name first_module))
...
let lookup_variable (type_env : t) qname : Types.datatype =
  let var_from_env var (env : t) = Env.String.find env.var_env var in
  let var_from_module var (module_t : Types.module_t) =
    StringMap.lookup var module_t.Types.fields
  in
  let resolve_res =
    resolve_qualified_name type_env qname var_from_env var_from_module
  in
...
let find_variable (env : t) qname : Types.datatype option =
  try Some (lookup_variable env qname) with
  | VariableNotFound _ -> None
  | ModuleNotFound _ -> None


let lookup_tycons_with_orig_path (type_env : t) qname :
    QualifiedName.t option * Types.tycon_spec =
  let type_from_env tycon (env : t) = Env.String.find env.tycon_env tycon in
  let type_from_module tycon (module_t : Types.module_t) =
    StringMap.lookup tycon module_t.Types.tycons
  in
  let resolve_res =
    resolve_qualified_name type_env qname type_from_env type_from_module
  in
  match resolve_res with
...
let find_tycons (env : t) qname : Types.tycon_spec option =
  try Some (snd (lookup_tycons_with_orig_path env qname)) with
  | TyConsNotFound _ -> None
  | ModuleNotFound _ -> None


let lookup_module_with_orig_path (type_env : t) qname :
    QualifiedName.t option * Types.module_t =
  let module_from_env var (env : t) = Env.String.find env.module_env var in
  let module_from_module var (module_t : Types.module_t) =
    StringMap.lookup var module_t.Types.modules
  in
  let resolve_res =
    resolve_qualified_name type_env qname module_from_env module_from_module
  in
  match resolve_res with
...
let open_module module_qname env_lookup_from env_add_to =
  let orig_path_opt, module_t =
    lookup_module_with_orig_path env_lookup_from module_qname
  in
  let orig_path =
    match orig_path_opt with
    | None -> Some module_qname
    | Some prefix_path -> Some (QualifiedName.append prefix_path module_qname)
  in
  let extend sm env =
    StringMap.fold
      (fun k v env -> Env.String.bind env (k, (orig_path, v)))
      sm env
  in
...
let bind_effects (env : t) eff = {env with effect_row= eff}
