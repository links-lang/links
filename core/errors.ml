...
let resolve_pos_expr pos =
  let open Position.Resolved in
  let pos = resolve pos in
  start pos, source_expression pos
...
let format_exception =
  ...
  | RichSyntaxError s ->
      ("*** Parse error: " ^ s.filename ^ ":"
       ...
       ^ "   " ^ s.marker)
  | SugarError (pos, s) ->
      let pos, expr = resolve_pos_expr pos in
  ...
  | UnboundTyCon (pos, tycon) ->
      let pos, _ = resolve_pos_expr pos in
