...
let encode : string -> string = fun s ->
  let rec need_code i =
    if i < String.length s then
      match s.[i] with
        | ' ' -> true
  ...
  in
  if need_code 0 then
    let len = compute_len 0 0 in Bytes.to_string (copy_code_in (Bytes.create len) 0 0)
  else 
    s
...
let string_starts_with : string -> string -> bool = fun s pref ->
...
let match_string : Str.regexp -> Str.regexp  -> string -> string = fun re1 re2 str ->
  try
