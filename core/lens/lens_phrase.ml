...
let equal v1 v2 = InfixAppl (Binary.Name "==", v1, v2)

let not' v1 = UnaryAppl (Unary.Name "!", v1)
...
let rec eval expr get_val =
  ...
  | InfixAppl (op, a1, a2) ->
      ...
      begin
          match (Binary.to_string op) with
          ...
          | "<" -> box_bool (unbox_int a1 < unbox_int a2)
          | _ as v -> failwith ("Unknown binary operation " ^ v ^ ".")
      end
  | TupleLit l -> eval (List.hd l) get_val
  | UnaryAppl (op, arg) ->
      begin
          match Unary.to_string op with
          | "!" ->
