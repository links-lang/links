...
sig
  ...
  = struct
    ...
    let error_policy () = { (Types.Print.default_policy ()) with Types.(* TODO put this back (Samo) Print.*)hide_fresh = false }
    ...
    let type_apply pos lexpr ty tys =
      ...
      let quant_policy () = { (error_policy ()) with Types.(* TODO put this back (Samo) Print.*)quantifiers = true } in
...
(* tab() ^ code (show_type rt)) *)
   ...
   let inconsistent_quantifiers ~pos ~t1:l ~t2:r =
     let policy () = {(Types.Print.default_policy ()) with Types.(* TODO here as well (~S) Print.*)quantifiers = true} in
    ...
    let escaped_quantifier ~pos ~var ~annotation ~escapees =
      ...
      let policy () = { (error_policy ()) with Types.(* TODO and here (~S) Print.*)quantifiers = true } in
