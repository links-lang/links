...
sig
  ...
  = struct
    ...
    let error_policy () = { (TP.default_policy ()) with TP.hide_fresh = false }
    ...
    let show_type   = TP.string_of_datatype ~policy:error_policy ~refresh_tyvar_names:false
    let show_row    = TP.string_of_row      ~policy:error_policy ~refresh_tyvar_names:false
    let show_effectrow row = "{" ^ (TP.string_of_row ~policy:error_policy ~refresh_tyvar_names:false row) ^ "}"
    let show_type_arg = TP.string_of_type_arg ~policy:error_policy ~refresh_tyvar_names:false

    (* Wrappers for generating type variable names *)
    let build_tyvar_names =
      TP.build_tyvar_names ~refresh_tyvar_names:true FreeTypeVars.free_bound_type_vars
    let add_rowvar_names =
      TP.add_tyvar_names FreeTypeVars.free_bound_row_type_vars
    let add_typearg_names =
      TP.add_tyvar_names FreeTypeVars.free_bound_type_arg_type_vars
    ...
    let type_apply pos lexpr ty tys =
      ...
      let quant_policy () = { (error_policy ()) with TP.quantifiers = true } in
      let ppr_type  = TP.string_of_datatype ~policy:quant_policy ~refresh_tyvar_names:false ty in
...
(* tab() ^ code (show_type rt)) *)
   ...
   let inconsistent_quantifiers ~pos ~t1:l ~t2:r =
     let policy () = {(TP.default_policy ()) with TP.quantifiers = true} in
     let typ = TP.string_of_datatype ~policy in
    ...
    let escaped_quantifier ~pos ~var ~annotation ~escapees =
      ...
      let policy () = { (error_policy ()) with TP.quantifiers = true } in
      let display_ty (var, ty) =
        Printf.sprintf "%s: %s" var
          (TP.string_of_datatype ~policy ~refresh_tyvar_names:false ty) in
      let displayed_tys =
      ...
      match (expr : phrasenode) with
                                          ...
                                          | `Present t ->
                                              if StringMap.mem label field_env then
                                                failwith ("Could not extend record "^ expr_string (erase r)^" (of type "^
                                                            TP.string_of_datatype rtype^") with the label "^
                                                            label^
                                                            " (of type"^TP.string_of_datatype (`Record (field_env, Unionfind.fresh `Closed, false))^
                ...
                StringMap.iter (fun v _ ->
                                if not (List.mem v vs) then
                                  ...
                                  if Types.Unl.can_type_be t then
                                    ...
                                    Gripers.die pos ("Variable " ^ v ^ " of linear type " ^ TP.string_of_datatype t ^ " is used in a non-linear function literal."))
                               (usages body)
              else () in
        ...
        | Spawn (k, given_loc, p, old_inner) ->
            ...
            if not (Types.Unl.can_type_be (typ p)) then
              Gripers.die pos ("Spawned processes cannot produce values of linear type (here " ^ TP.string_of_datatype (typ p) ^ ")");
                  ...
                  | `ForAll (_, (`Function _ | `Lolli _)) as t ->
                     begin
                       ...
                       | tyargs, (`Function (fps, fe, rettyp) | `Lolli (fps, fe, rettyp) as t) ->
                          ...
                          let arg_vars = Types.TypeVarSet.union (FreeTypeVars.free_type_vars fps) (FreeTypeVars.free_row_type_vars fe) in
                  ...
                  | `ForAll (_, `Record _) as t ->
                      ...
                      begin
                        ...
                        | Some (`Present t) ->
                          ...
                          let vars = FreeTypeVars.free_type_vars t in
        ...
        | Handle { sh_expr = m; sh_value_cases = val_cases; sh_effect_cases = eff_cases; sh_descr = descr; } ->
         ...
         (operations', rho, dual)
           ...
           (* Printf.printf "result: %s\ninner_eff: %s\nouter_eff: %s\n%!" (TP.string_of_datatype rt) (TP.string_of_row inner_eff) (TP.string_of_row outer_eff); *)
      ...
      | Fun { fun_binder = bndr; fun_linearity = lin;
          ...
          let context_body, ft, quantifiers =
            match t_ann with
              ...
              | Some t ->
                  (* Debug.print ("t: " ^ TP.string_of_datatype t); *)
                  ...
                  (* Debug.print ("ft_mono: " ^ TP.string_of_datatype ft_mono); *)
                  let () = unify pos ~handle:Gripers.bind_fun_annotation (no_pos shape, no_pos ft_mono) in
                    (* Debug.print ("return type: " ^TP.string_of_datatype (TypeUtils.concrete_type return_type)); *)
              ...
              StringMap.iter (fun v _ ->
                              if not (List.mem v vs) then
                                ...
                                if Types.Unl.can_type_be t then
                                  ...
                                  Gripers.die pos ("Variable " ^ v ^ " of linear type " ^ TP.string_of_datatype t ^
                 ...
                 let inner =
                   match t_ann with
                     ...
                     | Some t ->
                         ...
                         in
                         (* Debug.print ("ft: " ^ TP.string_of_datatype ft); *)
                         (* make sure the annotation has the right shape *)
                                          ...
                                          if not (StringSet.mem v vs) then
                                            ...
                                            if Types.Unl.can_type_be t then
                                              ...
                                              Gripers.die pos ("Use of variable " ^ v ^ " of linear type " ^
                                                                 TP.string_of_datatype t ^ " in unlimited function binding.")
                                          else ())
                                         (usages body);
