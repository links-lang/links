    ...
    let e, t, usages =
      match (expr : phrasenode) with
        ...
        | Query (range, p, _) ->
            ...
            let () = if Settings.get  Basicsettings.Shredding.relax_query_type_constraint then ()
        ...
        | Iteration (generators, body, where, orderby) ->
            ...
            let () =
              if is_query && not (Settings.get  Basicsettings.Shredding.relax_query_type_constraint) then
