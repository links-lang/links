...
module SugarConstructors (Position : Pos)
  ...
  let switch_fun_lit ?(ppos=dp) ?args ?(location=loc_unknown) linearity pat switch_funlit_body =
    with_pos ppos (FunLit (args, linearity, SwitchFunlit (pat, switch_funlit_body), location))
  ...
  let switch_fun_binding ?(ppos=dp) sig_opt ?(unsafe_sig=false) ((linearity, frozen), bndr, arg, location, blk) =
    let fun_signature = datatype_opt_of_sig_opt sig_opt bndr in
      with_pos ppos (Fun { fun_binder = binder bndr;
                           fun_linearity = linearity;
                           fun_definition = ([], SwitchFunlit (arg, blk));
                           fun_location = location;
                           fun_signature;
                           fun_frozen = frozen;
                           fun_unsafe_signature = unsafe_sig })
