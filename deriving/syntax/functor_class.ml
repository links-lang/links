...
| <:ctyp< [= $list:row$ ] >>   -> List.exists (fun [ MLast.RfTag _ _ targs -> List.exists contains_tvars targs
                                                   | MLast.RfInh _ -> False
                                                   ]) row
| <:ctyp< '$a$ >>              -> True
...
value rec gen_function loc = fun [
...
and gen_n loc types = 
    let vnames = List.map (Printf.sprintf "v%d") (range 0 (List.length types - 1)) in
      List.map2 
        ...
        types
;
value gen_case (loc, name, params) = 
match params with  [
...
value gen_instance loc tname param ctype =
...
instantiators.val :=   [("Functor"    , gen_instances):: instantiators.val];
