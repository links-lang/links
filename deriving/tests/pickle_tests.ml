...
module Test (P : Pickle.Pickle) =
struct
  let test v = P.unpickleS (P.pickleS v) = v
end

let sum = begin
  let module T = Test (Pickle_sum) in
    assert (T.test S0);
...
let r1 = begin
  let module T = Test (Pickle_r1) in
    assert (T.test {r1_l1 = max_int - 10; r1_l2 = min_int + 10});
end

let intseq = begin
  let module T = Test (Pickle_intseq) in
    assert (T.test INil);
    assert (T.test (ICons (10, ICons (20, ICons (30, INil)))));
end

let seq = begin
  let module T = Test (Pickle_seq (Pickle.Pickle_bool)) in
    assert (T.test Nil);
    assert (T.test (Cons (true, Cons (false, Cons (true, Nil)))));
end

let uses_seqs = begin
  let module T = Test (Pickle_uses_seqs) in
...
let poly1 = begin
  let module T = Test (Pickle_poly1) in
    assert (T.test `T0);
    assert (T.test (`T1 (-1231)));
end

let poly2 = begin
  let module T = Test (Pickle_poly2) in
    assert (T.test (P (10, `T1 11, 12.0)));
end

let poly3 = begin
  let module T = Test (Pickle_poly3) in
    assert (T.test `Nil);
    assert (T.test (`Cons (1, `Cons (2, `Cons (3, `Nil)))));
end

let poly3b = begin
  let module T = Test (Pickle_poly3b) in
    assert (T.test (10, `Nil, `F));
    assert (T.test (0, `Cons (10, `Cons (11, `Cons (12, `Nil))), `F));
end

let poly7 = begin
    let module T  = Test(Pickle_poly7(Pickle.Pickle_bool)) in
    let module T' = Test(Pickle_poly8(Pickle.Pickle_int))  in
...
let poly10 = begin
  let module T = Test (Pickle_poly10) in
    assert (T.test `F);
    assert (T.test `Nil);
...
let ff1 = begin
  let module T = Test(Pickle_ff1(Pickle.Pickle_bool)) in    
    assert (T.test (F (true,false)));
    assert (T.test (G 435));
end

let ff2 = begin
  let module T = Test(Pickle_ff2(Pickle.Pickle_bool)(Pickle.Pickle_int)) in
    assert (T.test (F1 (F2 (Nil, 10, None))));
    assert (T.test (F1 (F2 (Cons (true, Cons (false, Nil)), 10, Some 14))));
end

let tup0 = begin
  let module T = Test (Pickle_tup0) in
    assert (T.test ());
end

let tup2 = begin
  let module T = Test (Pickle_tup2) in
    assert (T.test (10, 10.0));
    assert (T.test (max_int, -10.0));
end

let tup3 = begin
  let module T = Test (Pickle_tup3) in
    assert (T.test (0,12.3,true));
    assert (T.test (min_int,-12.3,false));
end

let tup4 = begin
  let module T = Test (Pickle_tup4) in
    assert (T.test (0,0,true,()));
    assert (T.test (min_int,max_int,false,()));
end

let t = begin
  let module T = Test (Pickle_t) in
