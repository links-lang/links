...
module Symbols =
  struct
	...
	("s_" ^
         mapstrcat "_" 
           (fun ch ->
             if (Utility.Char.isWord ch) 
	     then
               String.make 1 ch
             else if CharMap.mem ch words 
	     then
...
let subst_ident n = 
  if StringMap.mem n ident_substs 
  then StringMap.find n ident_substs
  else n
      
let subst_primitive n =
  if Lib.is_primitive n 
  then "_"^n
  else n
  ...
  struct
	...
	method value : Ir.value -> query_value = function
	  ...
	  | `SplicedVariable var -> 
		...
		| Some var_name -> ( 
		    match get_primitive_name var_name with
...
module BoxingCamlTranslator = Translator (CamlBoxer)
module NonBoxingCamlTranslator = Translator (FakeBoxer)
...
let ml_of_ir cps box no_prelude env prelude (bs, tc) =
  ...
  let c =
    ...
    then 
      let t =
        if box 
	then new BoxingCamlTranslator.cps env
        else new NonBoxingCamlTranslator.cps env
      in 
      t#program comp
    else
      let t = 
        if box 
	then new BoxingCamlTranslator.direct env
        else new NonBoxingCamlTranslator.direct env
      in 
      t#program comp
  in
