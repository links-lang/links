...
let add_attribute : Value.t * Value.t -> Value.t -> Value.t =
  let rec filter = fun name -> function
    | [] -> []
    | Attr (s, _) :: nodes when s = name -> filter name nodes
    | NsAttr (ns, s, _) :: nodes when ns ^ ":" ^ s = name -> filter name nodes
    | node :: nodes -> node :: filter name nodes
  in
  fun (name,value) ->
    let name = unbox_string name
    and value = unbox_string value
    in let new_attr = match String.split_on_char ':' name with
          | [ n ] -> Attr(n, value)
          | [ ns; n ] -> NsAttr(ns, n, value)
          | _ -> failwith ("Attribute-name con only contain one colon for namespacing. Multiple found: " ^ name)
    in function
    | `XML (Node (tag, children))       -> `XML (Node (tag, new_attr :: filter name children))
    | `XML (NsNode (ns, tag, children)) -> `XML (NsNode (ns, tag, new_attr :: filter name children))
  ...
  *)
  ...
  (p1 (function
         | `List [`XML (Node (_, children))] ->
           let children = List.filter (function (Node _) -> true | (NsNode _) -> true | _ -> false) children in
             `List (map (fun x -> `XML x) children)
         | `List [ `XML (NsNode (_, _, children)) ] ->
           let children = List.filter (function (Node _) -> true | (NsNode _) -> true | _ -> false) children in
             `List (map (fun x -> `XML x) children)
  ...
  "attribute",
  (p2 (fun elem attr ->
    let attr = unbox_string attr
    in let find_attr = fun cs -> (try match List.find (function
      | Attr (k, _) when k = attr -> true
      | NsAttr (ns, k, _) when ns ^ ":" ^ k = attr -> true
      | _ -> false
    ) cs with
      | Attr (_, v) -> `Variant ("Just", box_string v)
      | NsAttr (_, _, v) -> `Variant ("Just", box_string v)
      | _ -> failwith "Internal error in `attribute'"
    with NotFound _ -> `Variant ("Nothing", `Record []))
    in match elem with
      | `List [ `XML (Node (_, children)) ] -> find_attr children
      | `List [ `XML (NsNode (_, _, children)) ] -> find_attr children
  ...
  (p3 (fun name attrList children -> match (attrList, children) with
    | (`List attrs, `List cs) -> `XML (Node (unbox_string(name), List.map (function
        | (`XML x) -> x
        | _ -> failwith "non-XML in makeXml"
      ) cs @ (List.map (function
          | `Record [ ("1", key); ("2", value) ] -> Attr (unbox_string(key), unbox_string(value))
  ...
  (p1 (fun v ->
         match v with
           | `List [ `XML (Node (name, _)) ]      -> box_string name
           | `List [ `XML (NsNode (_, name, _)) ] -> box_string name
  ...
  (p1 (fun v -> match v with
    | `List [ `XML (NsNode (ns, _, _)) ] -> box_string ns
    | `List [ `XML (Node (_, _)) ]       -> box_string ""
  ...
  (p1 (fun v ->
    let attr_to_record = function
      | (Attr (name, value)) -> `Record [("1", box_string name); ("2", box_string value)]
      | (NsAttr (ns, name, value)) -> `Record [ ("1", box_string (ns ^ ":" ^ name)); ("2", box_string value) ]
      | _ -> assert false
    and is_attr = function
      ...
      | _ -> false
    in match v with
      | `List [ `XML (Node (_, children)) ]      -> `List (map attr_to_record (filter is_attr children))
      | `List [ `XML (NsNode (_, _, children)) ] -> `List (map attr_to_record (filter is_attr children))
  ...
  (p1 (fun v ->
         match v with
           | `List [`XML(Node(_, children))] ->
               `List (map (fun x -> `XML(x)) (filter (function (Node _) -> true | (NsNode _) -> true | _ -> false) children))
           | `List [`XML(NsNode(_, _, children))] ->
               `List (map (fun x -> `XML(x)) (filter (function (Node _) -> true | (NsNode _) -> true | _ -> false) children))
...
(* FIXME: should these functions return a Maybe Char/Maybe String? *)
  ...
  ("strContains",
    (p2 (fun s c -> box_bool (String.contains (unbox_string s) (unbox_char c))),
