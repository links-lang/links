...
let test_data_2 = Sorted.construct_full ~columns:["a"; "b"; "str";]
    ~plus:[
      [box_bool true; box_int 5; box_string "abc"];
      [box_bool false; box_int 10; box_string "0123"];
    ]
...
let test_data_3 = Sorted.construct_full ~columns:["C"; "B"; "A";]
    ~plus:[
      [box_bool true; box_int 5; box_string "abc"];
      [box_bool true; box_int 3; box_string "this"];
      [box_bool false; box_int 9; box_string "0123"];
...
let test_data_4 = Sorted.construct_full ~columns:["B"; "A";]
    ~plus:[
...
let test_data_5 = Sorted.construct_full ~columns:["B"; "D";]
    ~plus:[
...
let test_data_7 = Sorted.construct_full ~columns:["B"; "D";]
    ~plus:[
      [box_int 2; box_string "data"];
      [box_int 14; box_string "exists"];
    ]
...
let test_construct_set _test_ctx =
  ...
  assert_equal (Sorted.plus_rows recs) (Array.of_list [
      [box_bool false; box_int 9; box_string "0123"];
      [box_bool true; box_int 3; box_string "this"];
      [box_bool true; box_int 5; box_string "abc"];
    ];);
...
let test_project _test_ctx =
  ...
  (* let str = SortedRecords.to_string_tabular recs in
     print_endline ("\n" ^ str); *)
  assert_equal recs (Sorted.construct_full ~columns:["str"; "b"]
                       ~plus:[
...
let test_compare _test_ctx =
  ...
  let cmp i = (Sorted.Simple_record.compare (Array.get (Sorted.plus_rows recs) i) find) in
  assert ((cmp 1) = 0);
  assert ((cmp 0) < 0);
  assert ((cmp 2) > 0)
...
let test_find_set _test_ctx =
  ...
  let test set find = Sorted.find (Sorted.construct ~records:set) ~record:find in
...
let test_find_all_ind_1 test_ctx =
  ...
  let (b,e) = Sorted.Simple_record.find_all_index (Sorted.plus_rows data) ~record:find in
  LensTestHelpers.print_verbose test_ctx (Format.asprintf "%a" Sorted.pp_tabular data);
...
let test_find_all_1 test_ctx =
  let data = test_data_4 in
  let find = [box_int 9] in
  let rows = Sorted.Simple_record.find_all_record (Sorted.plus_rows data) ~record:find in
  LensTestHelpers.print_verbose test_ctx (Format.asprintf "%a" Sorted.pp_tabular data);
  LensTestHelpers.print_verbose test_ctx (Format.asprintf "%a" Sorted.pp_tabular (
    Sorted.construct_full ~columns:(Sorted.columns data) ~plus:(Array.to_list rows) ~neg:[]));
...
let test_join_1 test_ctx =
  ...
  LensTestHelpers.print_verbose test_ctx (Format.asprintf "%a" Sorted.pp_tabular data_l);
  LensTestHelpers.print_verbose test_ctx (Format.asprintf "%a" Sorted.pp_tabular data_r);
  let joined = Sorted.join data_l data_r ~on:["B", "B", "B"] in
  LensTestHelpers.print_verbose test_ctx (Format.asprintf "%a" Sorted.pp_tabular joined);
  assert (Sorted.plus_rows joined = Array.of_list [
...
let test_join_neg_1 test_ctx =
  ...
  assert (Sorted.neg_rows joined = Array.of_list [
      [box_int 3; box_string "this"; box_string "in"];
      [box_int 5; box_string "abc"; box_string "other"];
      [box_int 14; box_string "9012"; box_string "exists"];
    ])
