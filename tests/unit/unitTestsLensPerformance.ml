...
let skip_long_perf_opt = Conf.make_bool "skip_long_perf" true "Skip long running performance tests."
...
let test_join_five test_ctx n =
  ...
  let l8 = LensTestHelpers.select_lens l7 (Phrase.equal (Phrase.var "b") (Phrase.Constant.int 10)) in
  (* run tests *)
...
let create_join_n_lens test_ctx db n rows =
  ...
  let ls = List.map (fun i ->
      ...
      let name = "t_" ^ stri in
      let fds = "p_" ^ stri ^ " -> p_" ^ string_of_int (i + 1) ^ " c_" ^ stri ^ "_2" in
      let cols = "p_" ^ stri ^ " p_" ^ string_of_int (i + 1) ^ " c_" ^ stri ^ "_2" in
  ...
  let l = LensTestHelpers.select_lens l (Phrase.equal (Phrase.var "p_2") (Phrase.Constant.int 10)) in
  l

let cleanup_join_n_lens test_ctx db n _rows =
  let r = LensTestHelpers.range 1 n in
  let _ = List.map (fun i ->
...
let benchmark_nr_of_lenses_add test_ctx =
  ...
  let _ = List.map (fun i ->
      ...
      let _put = box_list (r :: List.tl res) in
      (* let r = LensTestHelpers.time_query false (fun () -> lens_put l put None) in
         let _ = LensTestHelpers.print_verbose test_ctx (string_of_value r) in *)
      cleanup_join_n_lens test_ctx db i
    ) r in
...
let test_join_five_remove test_ctx n =
  ...
  let res = Lens.Value.lens_get l8 in
  let res = unbox_list res in
  let rm = List.filter (fun r ->
      1 = List.length (List.filter (fun r' -> Record.match_on r r' ~on:["c"]) res)
    ) res in
  (* let _ = print_endline (string_of_value (box_list rm)) in *)
  let rm = List.hd rm in
  let res = List.filter (fun r -> not (Record.match_on r rm ~on:["a"])) res in
  let _res = box_list res in
...
let test_join_five_update test_ctx n =
  ...
  let res = Lens.Value.lens_get l8 in
  let res = unbox_list res in
  let res = Record.set (List.hd res) ~key:"h" ~value:(box_int (1)) :: List.tl res in
  let _res = box_list res in
  (* let r = LensTestHelpers.time_query false (fun () -> lens_put l8 res None) in
     let _ = LensTestHelpers.print_verbose test_ctx (string_of_value r) in *)
  (* cleanup *)
