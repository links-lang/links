  ...
  message_callback : function(data) {
    ...
    switch (parsed.opcode) {
      ...
      case "GET_LOST_MESSAGES":
        ...
        clientA.send_orphan_messages();
        tests.sendLostMessageResponse(clientB.ws, remote_ep, ret);
        clientB.lost_msgs = undefined;
        break;
...
let clientC = {
  ...
  deliver_message : function(msg) {
    if (!clientC.lost_messages_delivered) {
      console.log("storing orphan: " + JSON.stringify(msg));
      clientC.orphans.unshift(msg);
    } else {
  ...
  adopt_orphans : function() {
    let orphans_length = clientC.orphans.length;
    for (var i = 0; i < orphans_length; i++) {
      clientC.deliver_message(clientC.orphans.pop());
    }
    clientC.orphans = undefined;
  },
  ...
  message_callback : function (data) {
    ...
    switch(parsed.opcode) {
      ...
      case "DELIVER_LOST_MESSAGES":
        ...
        clientC.lost_messages_delivered = true;
        for (var i = 0; i < msgs_len; i++) {
          clientC.deliver_message(msgs.pop());
        }
        clientC.adopt_orphans();
        break;
      case "SESSION_MESSAGE_DELIVERY":
