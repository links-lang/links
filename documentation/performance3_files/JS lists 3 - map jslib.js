...
function _lsCons(head, tail) { return { _label: '::', _head: head, _tail: tail }; }
var lsCons = _lsCons;
...
function _lsHead(v) { return v._label === '[]' ? _error('head') : v._head; }
function _lsTail(v) { return v._label === '[]' ? _error('tail') : v._tail; }
var lsHead = _lsHead;
var lsTail = _lsTail;

function _lsLast(xs) {
	//return lsNil;
	if (xs._label === '[]') { _error('last'); }
	var out = xs._head;
	while (xs._label !== '[]') {
		out = xs._head;
		xs = xs._tail;
...
function _lsMap(f, xs) {
	...
	var arr = [];
	while (xs._label !== '[]') {
		arr.push(f(xs._head, _idy)); // f doesn't return
		xs = xs._tail;
	}
	return _lsFromArray(arr);
}
