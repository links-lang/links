...
function _yield(f, args, k) { // args in a linked list?
	++_yieldCount;
...
function _yield2(f) {
	...
	if (_yieldCount == _yieldGranularity) {
		...
		if (_handlingEvent) {
			...
			var current_pid = _current_pid;
			window.setZeroTimeout(function() { _current_pid = current_pid; return f()});
		}
	} else {
		return f();
	}
...
function _yieldCont(k, arg) {
	...
	if (_yieldCount == _yieldGranularity) {
		...
		if (_handlingEvent) {
			...
			var current_pid = _current_pid;
			window.setZeroTimeout(function() { _current_pid = current_pid; k(arg) });
		}
	} else {
		//~ if (k === undefined)
			//~ _error("undefined");
...
var lsNil = null;//{ _label: '[]' };//, _head: {}, _tail: {}};
...
function _lsHead(v) { return v === null ? _error('head') : v._head; }
...
// if you could create String Objects like new String("test"), re-use those and use those in the comparisons, that would be even faster, because the JS engine would only need to do a pointer-comparison
...
function _objectEq(l, r) {
	return l._label == r._label && l._value == r._value;
}
