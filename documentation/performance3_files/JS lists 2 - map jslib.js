...
//    _debug("compiled regex: " + r);
    return (new RegExp(r)).test($str(s)); // makes garbage
}
...
function _yield(f, args, k) {
	...
	if (_yieldCount == _yieldGranularity) {
		_yieldCount = 0;
		if (_handlingEvent) {
			_theContinuation.v = function () {return f.apply(f, ___append(args, k));}; throw _theContinuation;
...
function _yieldCont(k, arg) {
	...
	if (_yieldCount == _yieldGranularity) {
		_yieldCount = 0;
		if (_handlingEvent) {
			//~ throw new _Continuation(function () { k(arg) }); // makes garbage
...
var lsNil = {'_label':'Nil', '_value':{}};

function _lsCons(hd,tl) { return { '_label':'Cn', '_value':{'1': hd, '2': tl} }; }
//var lsCons = LINKS.kify(_lsCons);
...
//var lsFromArray = LINKS.kify(_lsFromArray);
...
var lsLength = LINKS.kify(_lsLength);


function _lsHead(v) { return v._label === 'Nil' ? _error('head') : v._value["1"]; }
function _lsTail(v) { return v._label === 'Nil' ? _error('tail') : v._value["2"]; }
var lsHead = LINKS.kify(_lsHead);
var lsTail = LINKS.kify(_lsTail);

function _lsLast(xs) {
	if (xs._label === 'Nil') { _error('last'); }
	var out = xs._value["1"];
	while (xs._label !== 'Nil') {
		out = xs._value["1"];
		xs = xs._value["2"];
...
var lsLast = LINKS.kify(_lsLast);

//~ function _lsMap(f, xs) {
	//~ var arr = [];
	//~ while (xs._label !== 'Nil') {
		//~ arr.push(f(xs._value["1"]));
		//~ xs = xs._value["2"];
	//~ }
	//~ return _lsFromArray(arr);
//~ }
//~ var lsMap = LINKS.kify(_lsMap);
...
//~ function _lsMapIgnore(f, xs) {
	...
	//~ return;
//~ }
//~ var lsMapIgnore = LINKS.kify(_lsMapIgnore);
...
function _lsAppend(xs, ys) {
	if (xs._label === 'Nil') { return ys; }
	var rootEl = _lsCons(xs._value["1"], lsNil);
	var curr = rootEl;
	xs = xs._value["2"];
	while (xs._label !== 'Nil') {
		curr._value["2"] = _lsCons(xs._value["1"], lsNil);
		xs = xs._value["2"];
		curr = curr._value["2"];
	}
	curr._value["2"] = ys;
...
var lsAppend = LINKS.kify(_lsAppend);
