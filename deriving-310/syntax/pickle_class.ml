      ...
      let inner = List.fold_right 
        (fun t exp -> 
           let m = match t with
             ...
             <:expr< try $exp$
                         with UnknownTag _ -> ($id:m$.unpickle x :> a Read.m) >>)
        ts
        <:expr< raise (UnknownTag (n, ($str:"Unexpected tag encountered during unpickling of "
                                       ^tname$))) >>
    in <:match_case< n,[x] -> $inner$ >>

    method private polycase extension_tag tagspec : Ast.match_case = match tagspec with
    ...
    | `Local t -> 
        (* TODO: Find a tag number for each type that is guaranteed
          ...
          <:match_case<
            ...
            W.allocate obj
            (fun thisid ->
               $bind ~loc$ ($id:self#local t$.pickle obj)
                 (fun mid -> 
                    (W.store_repr thisid
    ...
    method variant (tname, params as atype) (_, tagspec) : Ast.module_expr =
      ...
      in
        wrap
          ...
          ~picklers:(List.map (self#polycase extension_tag) tagspec)
          ~unpickler
